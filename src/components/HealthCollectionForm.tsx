...
import { 
  ...
  ChevronRight, Compass, Flame, Droplet, Apple, HelpCircle, Minimize2
} from "lucide-react";
...
const LOCALES: Record<string, any> = {
  ...
  ta: {
    ...
    wearableDetails: "கைக்கடிகાર சாதன தரவு",
    age: "வயದು (ஆண்டுகள்)",
    gender: "பாலினம்",
    height: "உயரம் (செ.மீ)",
    weight: "எடை (கிலோ)",
  ...
  hi: {
    ...
    dataCollection: "सामूहिक स्वास्थ्य डेटा",
  ...
  ml: {
    ...
    dataCollection: "ആരോഗ്യ വിവര ശേഖരണം",
  ...
  kn: {
    ...
    dataCollection: "ಮಾಹಿತಿ ಸಂಗ್ರಹಣೆ",
...
export default function HealthCollectionForm({ userId, onTimelineSaved }: HealthCollectionFormProps) {
  const [lang, setLang] = useState<"en" | "ta" | "hi" | "ml" | "kn">("en");
  ...
  const startListening = (index: number) => {
    ...
    recognition.onresult = (event: any) => {
      ...
      if (lang === "ta") localizedSaved = "������Ưͯ�ͪ�͟��: ";
      else if (lang === "hi") localizedSaved = "&0M ?/> />: ";
      else if (lang === "ml") localizedSaved = "0G*M*FA$M$?/?0?MA(M(A: ";
      else if (lang === "kn") localizedSaved = "������������: ";
  ...
  return (
    <div id="health-timeline-sandbox" className="space-y-6">
      ...
      {/* 2-Option selector + Multilingual floating buttons */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[#0A0A0A] border border-[#1A1A1A] p-4 rounded-2xl relative overflow-hidden">
        <div>
          <div className="text-[10px] text-emerald-400 font-mono tracking-widest uppercase flex items-center gap-1">
            ...
            Bio-Metrics Archival System
          </div>
          <h2 className="text-lg font-black text-white mt-1 uppercase tracking-tight">Clinical Intake Workspace</h2>
        </div>

        {/* Locale Selectors option box */}
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-slate-500 font-mono flex items-center gap-1">
            <Volume2 className="h-3.5 w-3.5" /> Language:
          </span>
      ...
      {false && voiceModeActive && (
        <div className="bg-[#0D0D0D] border border-emerald-550/20 p-5 rounded-2xl items-center justify-between flex flex-col md:flex-row gap-4 shadow-xl relative animate-pulse">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[9px] text-emerald-405 font-mono uppercase tracking-widest block">Voice AI Clinical Assistant Mode</span>
              <p className="text-xs font-bold text-slate-200 mt-0.5 leading-snug">
                Question {voiceQuestionIndex + 1} of {activeQuestionsList.length}: <span className="text-emerald-400 font-extrabold">{activeQuestionsList[voiceQuestionIndex].text}</span>
              </p>
              <p className="text-[10px] text-slate-550 italic mt-1 font-medium bg-black/40 px-2 py-0.5 rounded leading-normal">
          ...
          >
            <Minimize2 className="h-3.5 w-3.5" /> Stop Guidance
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          {false && !voiceModeActive && (
            <div className="bg-[#090909] border border-[#181818] p-4 rounded-xl flex items-center justify-between text-left gap-4">
              <div>
                <span className="text-[10px] text-emerald-450 font-bold tracking-widest uppercase block">{t.voicePrompt}</span>
                <p className="text-[11px] text-slate-500 mt-0.5">Assist elders or visually-impaired individuals in dictating health metrics smoothly.</p>
              </div>
              <button
                type="button"
              ...
              >
                <Volume2 className="h-4.5 w-4.5 text-emerald-400" /> Start Guideline Speak
              </button>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 bg-[#121212]/90 border border-[#222] p-1.5 rounded-2xl gap-1.5 my-2.5">
            ...
            >
              <Activity className="h-4 w-4" /> 🧬 Physical & Laboratory
            </button>
            <button
            ...
            >
              <Sparkles className="h-4 w-4 text-purple-400" /> 🧠 Mental & Voice/Chat
            </button>
          </div>
          ...
          <div className="bg-[#0A0A0A]/60 border border-[#1A1A1A] rounded-2xl p-6 shadow-xl space-y-6">
            ...
            {activeTab === "physical" ? (
              <>
                <div className="border-b border-[#1A1A1A] pb-4 flex justify-between items-start gap-4">
                  <div>
                    <h3 className="text-base font-black text-white uppercase tracking-tight">{t.dataCollection}</h3>
                    <p className="text-xs text-slate-500 mt-1">Populate baseline clinical, nutritional, and lifestyle metrics with timeline continuity.</p>
                  </div>
                ...
                <h4 className="text-xs font-black tracking-widest text-[#E0E0E0] uppercase flex items-center gap-2 pb-1.5 border-b border-[#151515]">
                  <span className="h-1.5 w-1.5 bg-emerald-500 rounded-full" /> {t.basicInfo}
                </h4>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">{t.age}</label>
                    <input
                      type="number"
                      required
                      value={basicInfo.age}
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">{t.gender}</label>
                    <select
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">{t.height}</label>
                    <input
                      type="number"
                      required
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">{t.weight}</label>
                    <input
                      type="number"
                      required
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">{t.bmi}</label>
                  ...
                  <div className="space-y-1 col-span-2 md:col-span-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">{t.occupation}</label>
                    <input
                      type="text"
              ...
              <div className="bg-[#0F0F0F] border border-[#1A1A1A] rounded-xl p-4 space-y-4">
                <h4 className="text-xs font-black tracking-widest text-[#E0E0E0] uppercase flex items-center gap-2 pb-1.5 border-b border-[#151515]">
                  <span className="h-1.5 w-1.5 bg-amber-500 rounded-full" /> Semi-Dynamic Biomarkers
                </h4>
                <p className="text-[10px] text-slate-500 leading-relaxed">
                  Update these clinical values monthly or via digital report extraction to enable long-term early warning signals.
                </p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5">
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">Blood Sugar (mg/dL)</label>
                    <input
                      type="number"
                      required
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">Systolic BP (mmHg)</label>
                    <input
                      type="number"
                      required
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">Diastolic BP (mmHg)</label>
                    <input
                      type="number"
                      required
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">Total Cholesterol</label>
                    <input
                      type="number"
                      required
              ...
              <div className="bg-[#0E0E0E] border border-[#1C1C1C] rounded-xl p-4 space-y-4 relative overflow-hidden">
                <div className="absolute top-0 right-0 py-1.5 px-3 bg-slate-900 border-l border-b border-[#222] text-emerald-400 font-mono text-[8px] uppercase font-bold tracking-widest rounded-bl-lg">
                  Omit to Skip
                </div>
                <h4 className="text-xs font-black tracking-widest text-[#E0E0E0] uppercase flex items-center gap-2 pb-1.5 border-b border-[#171717]">
                  <span className="h-1.5 w-1.5 bg-emerald-400 rounded-full animate-pulse" /> 🧬 Classical Medical Report Entry
                </h4>
                <p className="text-[10px] text-slate-500 leading-relaxed">
                  Enter secondary lab metrics manually below if OCR document extraction fails. Leave empty to skip.
                </p>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  <div className="space-y-1">
                    <label className="text-[9px] text-[#A0A0A0] font-bold uppercase tracking-wider block">Hemoglobin A1c (HbA1c%)</label>
                    <input
                      type="number"
                      step="0.1"
                      placeholder="e.g. 5.7"
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] text-[#A0A0A0] font-bold uppercase tracking-wider block">LDL Cholesterol (mg/dL)</label>
                    <input
                      type="number"
                      placeholder="e.g. 100"
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] text-[#A0A0A0] font-bold uppercase tracking-wider block">HDL Cholesterol (mg/dL)</label>
                    <input
                      type="number"
                      placeholder="e.g. 50"
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] text-[#A0A0A0] font-bold uppercase tracking-wider block">Triglycerides (mg/dL)</label>
                    <input
                      type="number"
                      placeholder="e.g. 150"
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] text-[#A0A0A0] font-bold uppercase tracking-wider block">Serum Creatinine (mg/dL)</label>
                    <input
                      type="number"
                      step="0.01"
                      placeholder="e.g. 0.9"
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] text-[#A0A0A0] font-bold uppercase tracking-wider block">ALT / SGPT (U/L)</label>
                    <input
                      type="number"
                      placeholder="e.g. 30"
                  ...
                  <div className="space-y-1 col-span-2 md:col-span-1">
                    <label className="text-[9px] text-[#A0A0A0] font-bold uppercase tracking-wider block">Thyroid TSH (mIU/L)</label>
                    <input
                      type="number"
                      step="0.01"
                      placeholder="e.g. 2.1"
              ...
              {(basicInfo.gender === "Female" || basicInfo.gender === "Female") && (
                <div className="bg-gradient-to-b from-[#110D16] to-[#0D0D0E] border border-fuchsia-900/20 rounded-xl p-4 space-y-4">
                  <h4 className="text-xs font-black tracking-widest text-[#E0E0E0] uppercase flex items-center gap-2 pb-1.5 border-b border-fuchsia-950/20">
                    <span className="h-2 w-2 bg-fuchsia-500 rounded-full animate-pulse" /> 🌸 Endocrine & Women's cycle manager
                  </h4>
                  <p className="text-[10px] text-fuchsia-400 leading-relaxed">
                    Personalized hormone balance notes, pregnancy surveillance, thyroid status and PCOS symptoms (Optional).
                  </p>
                  
                  <div className="grid grid-cols-2 gap-3.5">
                    <div className="space-y-1">
                      <label className="text-[9px] text-[#C084FC] font-bold uppercase block">Menstruation Cycle</label>
                      <select
                      ...
                      >
                        <option value="Regular">Regular Status (28-32 Days)</option>
                        <option value="Irregular">Irregular Cycle</option>
                        <option value="Menopause">Post-Menopausal Phase</option>
                        <option value="Not Stated">Omit Cycle Info</option>
                      </select>
                    </div>

                    <div className="space-y-1">
                      <label className="text-[9px] text-[#C084FC] font-bold uppercase block">PCOS Signs / Tested</label>
                      <select
                        value={womensHealth.pcos}
                        onChange={(e) => setWomensHealth(prev => ({ ...prev, pcos: e.target.value }))}
                        className="w-full bg-[#0A0710] border border-[#2E1E3B] text-xs text-fuchsia-200 px-3 py-2.5 rounded-lg outline-none cursor-pointer"
                      >
                        <option value="No">No active symptoms / Negative</option>
                        <option value="Yes">Yes (Confirmed Diagnostic / Signs)</option>
                        <option value="Untested">Untested / No concerns</option>
                      </select>
                    </div>

                    <div className="space-y-1">
                      <label className="text-[9px] text-[#C084FC] font-bold uppercase block">Hormone Balance</label>
                      <select
                        value={womensHealth.hormoneBalance}
                        onChange={(e) => setWomensHealth(prev => ({ ...prev, hormoneBalance: e.target.value }))}
                        className="w-full bg-[#0A0710] border border-[#2E1E3B] text-xs text-fuchsia-200 px-3 py-2.5 rounded-lg outline-none cursor-pointer"
                      >
                        <option value="Normal">Normal physiological homeostasis</option>
                        <option value="Imbalanced">Mild hormonal imbalance</option>
                        <option value="Omit">Not entered</option>
                      </select>
                    </div>

                    <div className="space-y-1">
                      <label className="text-[9px] text-[#C084FC] font-bold uppercase block">Pregnancy Track Status</label>
                      <select
                        value={womensHealth.pregnancyStatus}
                        onChange={(e) => setWomensHealth(prev => ({ ...prev, pregnancyStatus: e.target.value }))}
                        className="w-full bg-[#0A0710] border border-[#2E1E3B] text-xs text-fuchsia-200 px-3 py-2.5 rounded-lg outline-none cursor-pointer"
                      >
                        <option value="No">Not Pregnant / Negative</option>
                        <option value="Yes">Yes (Active Gestation - Track closely)</option>
                        <option value="Nursing">Nursing / Lactation period</option>
                        <option value="Planning">Conception Planning window</option>
                      </select>
                    </div>

                    <div className="col-span-2 space-y-1">
                      <div className="flex justify-between items-center">
                        <label className="text-[9px] text-[#C084FC] font-bold uppercase block">Pregnancy & Thyroid Health Notes</label>
                        {womensHealth.pregnancyStatus === "Yes" && (
                          <span className="text-[8px] bg-emerald-500/20 text-emerald-300 px-1.5 py-0.5 rounded uppercase font-black tracking-widest animate-pulse">Prenatal Vigilance Active</span>
                        )}
                      </div>
                      <textarea
                        rows={2}
              ...
              <div className="bg-[#0F0F0F] border border-[#1A1A1A] rounded-xl p-4 space-y-4">
                <h4 className="text-xs font-black tracking-widest text-[#E0E0E0] uppercase flex items-center gap-2 pb-1.5 border-b border-[#151515]">
                  <span className="h-1.5 w-1.5 bg-emerald-500 rounded-full" /> {t.lifestyle}
                </h4>
                <div className="grid grid-cols-2 gap-3.5">
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      <label className="text-[10px] text-slate-500 font-bold uppercase">{t.sleepDuration}</label>
                      <span className="text-[10px] font-mono text-emerald-450 font-bold">{lifestyle.sleepDuration} hrs</span>
                    </div>
                    <input
                      type="range"
                      min="4"
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">{t.sleepQuality}</label>
                    <select
                      value={lifestyle.sleepQuality}
                      onChange={(e) => handleManualValueChange("lifestyle", "sleepQuality", e.target.value)}
                      className="w-full bg-[#050505] border border-[#1E1E1E] text-xs text-white px-3 py-2.5 rounded-lg outline-none cursor-pointer"
                    >
                      <option value="1">1 - Highly Interrupted</option>
                      <option value="2">2 - Light Sleep</option>
                      <option value="3">3 - Standard Rest</option>
                      <option value="4">4 - Good Restorative</option>
                      <option value="5">5 - Perfect Deep Recovery</option>
                    </select>
                  </div>

                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      <label className="text-[10px] text-slate-500 font-bold uppercase">{t.stressLevel}</label>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">{t.physicalActivity}</label>
                    <select
                      value={lifestyle.physicalActivity}
                      onChange={(e) => handleManualValueChange("lifestyle", "physicalActivity", e.target.value)}
                      className="w-full bg-[#050505] border border-[#1E1E1E] text-xs text-white px-3 py-2.5 rounded-lg outline-none cursor-pointer"
                    >
                      <option value="Walking">Walking / Jogging</option>
                      <option value="Yoga">Yoga & Flexibility</option>
                      <option value="Running">Aerobic Running</option>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">{t.sittingHours}</label>
                    <input
                      type="number"
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">{t.smoking}</label>
                    <select
                  ...
                  <div className="space-y-1 col-span-2">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">{t.alcohol}</label>
                    <select
              ...
              <div className="bg-[#0F0F0F] border border-[#1A1A1A] rounded-xl p-4 space-y-4">
                <h4 className="text-xs font-black tracking-widest text-[#E0E0E0] uppercase flex items-center gap-2 pb-1.5 border-b border-[#151515]">
                  <span className="h-1.5 w-1.5 bg-emerald-500 rounded-full" /> {t.nutrition}
                </h4>
                <div className="grid grid-cols-2 gap-3.5">
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">{t.mealPattern}</label>
                    <select
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">{t.water}</label>
                    <input
                      type="number"
                      step="0.5"
                  ...
                  <div className="space-y-1.5 col-span-2">
                    <div className="flex justify-between items-center">
                      <label className="text-[10px] text-slate-500 font-bold uppercase">{t.junkFood}</label>
                  ...
                  <div className="space-y-1.5 col-span-2">
                    <div className="flex justify-between items-center">
                      <label className="text-[10px] text-slate-500 font-bold uppercase">{t.sugar}</label>
              ...
              <div className="bg-[#0F0F0F] border border-[#1A1A1A] rounded-xl p-4 space-y-4">
                <h4 className="text-xs font-black tracking-widest text-[#E0E0E0] uppercase flex items-center gap-2 pb-1.5 border-b border-[#151515]">
                  <span className="h-1.5 w-1.5 bg-emerald-500 rounded-full" /> {t.medicalHistory}
                </h4>
                <div className="space-y-3">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label className="text-[10px] text-slate-500 font-bold uppercase">{t.existingDiseases}</label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] text-slate-500 font-bold uppercase">{t.previousDiagnosis}</label>
                      <input
                        type="text"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label className="text-[10px] text-slate-500 font-bold uppercase">{t.surgeries}</label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] text-slate-500 font-bold uppercase">{t.infections}</label>
                      <input
                        type="text"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label className="text-[10px] text-slate-500 font-bold uppercase">{t.allergies}</label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-1 invisible hidden">
                      <label className="text-[10px] text-slate-500 font-bold uppercase">{t.medications}</label>
                      <input
                        type="text"
                  ...
                  <div className="space-y-3 bg-[#080808] border border-[#1A1A1A] p-4 rounded-xl">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <div className="space-y-0.5">
                        <label className="text-xs text-white font-bold tracking-wide">Are you currently taking any prescription medications?</label>
                        <p className="text-[10px] text-slate-500">Configure active therapies to calculate chronic risk models and adherence logs.</p>
                      </div>
                    ...
                    {hasMedications && (
                      <div className="space-y-3 pt-3 border-t border-[#151515] duration-300">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div className="space-y-1">
                            <label className="text-[10px] text-slate-500 font-bold uppercase">Medicine Name</label>
                            <input
                              type="text"
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <div className="space-y-1">
                              <label className="text-[10px] text-slate-500 font-bold uppercase">Dosage</label>
                              <input
                                type="text"
                            ...
                            <div className="space-y-1">
                              <label className="text-[10px] text-slate-500 font-bold uppercase">Purpose</label>
                              <input
                                type="text"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          <div className="space-y-1">
                            <label className="text-[10px] text-slate-500 font-bold uppercase">Frequency</label>
                            <select
                              value={newMed.frequency}
                              onChange={(e) => {
                                const freq = e.target.value;
                                let defaultTimes = ["08:00"];
                                if (freq === "Twice Daily") defaultTimes = ["08:00", "20:00"];
                                else if (freq === "Thrice Daily") defaultTimes = ["08:00", "13:00", "20:00"];
                          ...
                          <div className="space-y-1">
                            <label className="text-[10px] text-slate-500 font-bold uppercase">Relation to Meals</label>
                            <select
                            ...
                            >
                              ...
                              <option value="No Relation">No Relation / Anytime</option>
                            </select>
                          </div>
                          <div className="space-y-1">
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[10px] text-slate-500 font-bold uppercase">Alarms / Reminder Times</label>
                          <div className="flex flex-wrap gap-2">
                            ...
                            {newMed.frequency === "Weekly" && (
                              <span className="text-[10px] text-slate-500 self-center">Fires on the scheduled weekday at choice time.</span>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex justify-end pt-1">
                          <button
                            ...
                            className="bg-emerald-500 hover:bg-emerald-600 active:scale-95 duration-200 text-black text-xs font-bold px-4 py-2 rounded-lg transition-transform shadow-lg shadow-emerald-500/10 cursor-pointer"
                          >
                            Add Medication
                          </button>
                        </div>
                        ...
                        {onboardingMedications.length > 0 && (
                          <div className="mt-3 bg-black/50 border border-[#1E1E1E] rounded-lg p-3 space-y-2">
                            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Staged Onboarding Medications:</span>
                              ...
                              {onboardingMedications.map((m, i) => (
                                <div key={i} className="flex justify-between items-center bg-[#070707] border border-[#151515] p-2.5 rounded-lg">
                                  <div className="min-w-0 pr-2">
                                    ...
                                    <p className="text-[9px] text-slate-500 truncate mt-0.5">Fires reminders at: {m.times.map(formatTime12Hour).join(", ")}</p>
                                  </div>
                                  <button
                                    type="button"
                                    onClick={() => {
                                    ...
                                    className="text-red-400 hover:text-red-300 text-[10px] font-bold px-2 py-1 rounded hover:bg-red-500/10 cursor-pointer transition-colors shrink-0"
                                  >
                                    Remove
                                  </button>
                                </div>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-500 font-bold uppercase">{t.familyHistory}</label>
                    <input
                      type="text"
              ...
              <div className="bg-[#0F0F0F] border border-[#1A1A1A] rounded-xl p-4 space-y-4">
                <h4 className="text-xs font-black tracking-widest text-[#E0E0E0] uppercase flex items-center gap-2 pb-1.5 border-b border-[#151515]">
                  <Upload className="h-4 w-4 text-emerald-400" /> {t.medicalReports}
                </h4>
                <p className="text-[11px] text-slate-500 leading-normal">
                  Drop clinical notes or paste pathology text values. The parser automatically extracts key indicators to fill forms.
                </p>
              ...
              <div className="bg-[#0F0F0F] border border-[#1A1A1A] rounded-xl p-4 space-y-4">
                <h4 className="text-xs font-black tracking-widest text-[#E0E0E0] uppercase flex items-center gap-2 pb-1.5 border-b border-[#151515]">
                  <Activity className="h-4 w-4 text-emerald-400 animate-pulse" /> {t.wearableDetails}
                </h4>
                <p className="text-[11px] text-slate-500 leading-normal">
                  Synchronize active biometric steps, sleep records, and average cardiac rates directly from Garmin, Fitbit, or Apple Watch.
                </p>

                {bluetoothError && (
                  <div className="bg-red-950/20 border border-red-900/30 p-3 rounded-lg text-red-400 text-[10px] leading-relaxed">
                    <strong>Notice:</strong> {bluetoothError}
                  </div>
                )}
                ...
                {wearableConnected ? (
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 text-left bg-black/40 border border-emerald-500/10 p-3.5 rounded-xl">
                      <div>
                        <span className="text-[8px] text-slate-500 uppercase block">{t.steps}</span>
                        <strong className="text-xs font-mono text-emerald-400 block mt-0.5">{wearableDetails.steps} steps</strong>
                      </div>
                      <div>
                        <span className="text-[8px] text-slate-500 uppercase block">{t.heartRate}</span>
                        <strong className="text-xs font-mono text-emerald-400 block mt-0.5">{wearableDetails.heartRate} bpm</strong>
                      </div>
                      <div className="col-span-2">
                        <span className="text-[8px] text-slate-550 uppercase block">{t.sleepCycle}</span>
                      ...
                      <div className="col-span-4 border-t border-[#131313] pt-1.5 mt-1.5">
                        <span className="text-[8px] text-slate-550 uppercase block">Wearable activity log summary</span>
                    ...
                    <div className="bg-[#070707] border border-[#161616] p-4 rounded-xl space-y-4">
                      <span className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider block border-b border-[#141414] pb-1.5">🎙️ Live Smartwatch Telemetry Tuner</span>
                      
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-1">
                          <div className="flex justify-between items-center text-[10px]">
                            <label className="text-slate-400 font-bold uppercase">Daily Step Counter</label>
                            <span className="font-mono text-emerald-400 font-bold">{wearableDetails.steps} steps</span>
                          </div>
                          <input
                            type="range"
                            min="1000"
                        ...
                        <div className="space-y-1">
                          <div className="flex justify-between items-center text-[10px]">
                            <label className="text-slate-400 font-bold uppercase">Average Cardiac BPM</label>
                            <span className="font-mono text-emerald-400 font-bold">{wearableDetails.heartRate} bpm</span>
                          </div>
                          <input
                            type="range"
                            min="40"
                        ...
                        <div className="col-span-1 sm:col-span-2 space-y-1">
                          <label className="text-[10px] text-slate-400 font-bold uppercase">Nightly Sleep REM Rhythm</label>
                          <select
                          ...
                          >
                            <option value="94% Healthy REM Deep Synchronization">94% Highly Synchronized (Optimal Deep REST)</option>
                            <option value="82% Deep sleep efficiency">82% Healthy (Standard Sleep recovery)</option>
                            <option value="64% Sleep cycles fragmented">64% Fragmented (Minor insomnia fatigue)</option>
                            <option value="42% Bad micro-arousals detected">42% Low Sleep Wave Efficiency (Critical vascular risk)</option>
                          </select>
                        </div>

                        <div className="col-span-1 sm:col-span-2 space-y-1">
                          <label className="text-[10px] text-slate-400 font-bold uppercase">Activity Narrative Logs</label>
                          <input
                            type="text"
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="w-full bg-[#1F1115] hover:bg-[#2A171D] border border-red-950 text-red-400 text-[10px] font-bold py-2 px-3 rounded-lg cursor-pointer transition-colors uppercase tracking-wider"
                      >
                  ...
                  <div className="space-y-3.5">
                    <button
                      ...
                      className="w-full bg-gradient-to-r from-emerald-600 to-teal-700 hover:brightness-110 text-white font-bold text-[11px] py-3 rounded-xl shadow-lg shadow-emerald-500/10 cursor-pointer transition-transform uppercase tracking-wider"
                    >
                      🚀 Sync Smartwatch Biometrics
                    </button>

                    <div className="relative flex py-1 items-center">
                      <div className="flex-grow border-t border-[#1C1C1C]"></div>
                      <span className="flex-shrink mx-3 text-[9px] text-slate-550 font-bold uppercase tracking-widest">or</span>
                      <div className="flex-grow border-t border-[#1C1C1C]"></div>
                    </div>
                    ...
                    >
                      {isConnectingBluetooth ? (
                        <>
                          ...
                          Pairing Active Bluetooth Device...
                        </>
                      ) : (
                        <>
                          <span>🌐</span> Pair Real Smartwatch via Web Bluetooth BLE
                        </>
                      )}
                    </button>
                  </div>
            ...
            <div className="space-y-6">
              <div className="border-b border-[#1A1A1A] pb-4 flex justify-between items-start gap-4">
                <div>
                  <h3 className="text-base font-black text-purple-300 uppercase tracking-tight">Anxiety & Stress Assessment</h3>
                  <p className="text-xs text-slate-500 mt-1">Simulate clinically-paired psychological indicators, cortisol stress, and voice inputs.</p>
                </div>
                <div className="text-[10px] bg-purple-500/10 text-purple-400 px-2.5 py-0.5 rounded border border-purple-500/20 font-bold uppercase tracking-wider leading-none">
                  Resilience AI active
                </div>
              </div>
              ...
              <div className="bg-[#0C0B11] border border-purple-900/20 p-5 rounded-xl shadow-lg relative overflow-hidden">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <span className="text-[9px] text-purple-400 font-mono tracking-widest uppercase block">Allostatic Balance</span>
                    <h4 className="text-sm font-extrabold text-[#E0E0E0] mt-1">Mental Health Resilience Score</h4>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl font-black text-purple-400 font-mono leading-none">{mentalScore}</div>
                    <span className="text-[9px] font-bold uppercase text-purple-500 tracking-wider">Index score</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 pt-4 border-t border-[#1F1E28] mt-4 text-center">
                  <div>
                    <span className="text-[8px] text-slate-500 block uppercase">Anxiety Level</span>
                    <strong className="text-xs font-mono text-purple-300">{mentalAnxiety}/10</strong>
                  </div>
                  <div>
                    <span className="text-[8px] text-slate-500 block uppercase">Perceived Stress</span>
                    <strong className="text-xs font-mono text-purple-300">{mentalStress}/10</strong>
                  </div>
                  <div>
                    <span className="text-[8px] text-slate-500 block uppercase">Risk State</span>
                    <strong className="text-xs font-bold text-emerald-400">Low-Moderate</strong>
                  </div>
                </div>
              </div>
              ...
              <div id="vocal-stress-analyzer-section" className="bg-[#0D0B12] border border-purple-500/20 rounded-xl p-4 space-y-4 relative overflow-hidden">
                <div className="absolute right-0 top-0 text-[8px] tracking-widest uppercase font-mono px-2 py-1 bg-purple-500/20 text-purple-300 rounded-bl-lg">
                  Paralinguistic Acoustic Modulate
                </div>
                <h4 className="text-xs font-black tracking-widest text-purple-300 uppercase flex items-center gap-2 pb-1.5 border-b border-purple-900/40 font-sans">
                  <Mic className="h-4 w-4 text-purple-400 rotate-12" /> Speech Stress & Anxiety Analyzer
                </h4>
                <p className="text-[11px] text-slate-400 leading-normal">
                  Perform a high-precision vocal test. This paralinguistic model measures micro-vibrational pitch jitter, amplitude shimmers, silences, and linguistic distressed cues to automatically evaluate stress.
                </p>
                ...
                {isVoiceTesting ? (
                  <div className="flex flex-col items-center justify-center py-6 bg-purple-950/20 border border-purple-500/10 rounded-xl space-y-3">
                    ...
                    <div className="text-center">
                      <p className="text-xs font-extrabold text-white">Answering Prompt: "How are you feeling right now?"</p>
                      <p className="text-[10px] text-purple-400 font-mono mt-1">Recording speech... {voiceTestSeconds}s remains</p>
                    </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    >
                      <Mic className="h-4 w-4" /> Start Speech Stress & Anxiety Analyzer
                    </button>
                  </div>
                )}
                ...
                {voiceTestTranscript && (
                  <div className="bg-[#050508] border border-purple-900/30 p-3.5 rounded-xl space-y-3.5">
                    <div>
                      <span className="text-[8px] text-purple-400 font-mono tracking-widest uppercase block mb-1">Acoustic Speech Transcription</span>
                    ...
                    {acousticResult && (
                      <div className="space-y-3">
                        <div className="border-t border-[#1C1625] pt-3">
                          <span className="text-[8px] text-purple-400 font-mono tracking-widest uppercase block mb-2">Simulated Acoustic Telemetry Insights</span>
                          
                          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 text-center">
                            <div className="bg-[#0F0D15] border border-[#1E172C] p-2 rounded-lg">
                              <span className="text-[8px] text-slate-500 block uppercase">Micro-vibrational Jitter</span>
                            ...
                            <div className="bg-[#0F0D15] border border-[#1E172C] p-2 rounded-lg">
                              <span className="text-[8px] text-slate-500 block uppercase">Decibel Shimmer</span>
                            ...
                            <div className="bg-[#0F0D15] border border-[#1E172C] p-2 rounded-lg">
                              <span className="text-[8px] text-slate-500 block uppercase">Speech Prosody Rate</span>
                              <strong className="text-xs font-mono text-purple-300">{acousticResult.speechRate} WPM</strong>
                            ...
                            <div className="bg-[#0F0D15] border border-[#1E172C] p-2 rounded-lg">
                              <span className="text-[8px] text-slate-550 block uppercase">Silent Hesitations</span>
                              <strong className="text-xs font-mono text-purple-300">{acousticResult.pauseDensity} pause(s)</strong>
                        ...
                        <div className="bg-purple-950/20 border border-purple-500/10 p-2.5 rounded-lg text-center">
                          <span className="text-[8px] text-purple-400 block font-mono uppercase">Linguistic Distressed Markers Detected</span>
                          <strong className="text-xs text-white block font-sans mt-0.5">{acousticResult.linguisticStress} flag(s) count</strong>
                          <p className="text-[9px] text-slate-400 leading-normal mt-1">
                            Calculated of stress keywords and mapped with physical <span className="text-purple-300 font-bold">{lifestyle.sleepDuration} hrs sleep</span> habits.
                          </p>
                        </div>

                        <div className="border-t border-[#1C1625] pt-2 text-center">
                          <span className="text-[8px] text-purple-400 block font-mono uppercase">Acoustic Stress Verdict</span>
              ...
              <div className="bg-[#0F1012] border border-[#1E1E1E] rounded-xl p-4 space-y-4">
                <h4 className="text-xs font-black tracking-widest text-[#E0E0E0] uppercase flex items-center gap-2 pb-1.5 border-b border-[#1A1A1A]">
                  <span className="h-1.5 w-1.5 bg-purple-400 rounded-full" /> Anxiety & Stress Monitor
                </h4>
                
                <div className="space-y-4 font-sans">
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      <label className="text-[10px] text-slate-400 font-bold uppercase">Anxiety Rate (Generalized Nervous Alert)</label>
                  ...
                  <div className="space-y-1.5 pt-2">
                    <div className="flex justify-between items-center">
                      <label className="text-[10px] text-slate-400 font-bold uppercase">Stress Load (Biological Cortisol Tension)</label>
              ...
              <div className="bg-[#0A0A0B] border border-purple-500/10 rounded-xl p-4 space-y-4">
                <div className="flex justify-between items-center border-b border-[#1A1A1A] pb-2">
                  <div>
                    <h4 className="text-xs font-black tracking-widest text-[#E0E0E0] uppercase flex items-center gap-1.5">
                      <span className="h-1.5 w-1.5 bg-indigo-400 rounded-full animate-ping" /> Counselor Assessment Room
                    </h4>
                    <p className="text-[9px] text-slate-550">Live Voice Transcription or Chat clinical consultation.</p>
                  </div>
                  {listeningMental && (
                    <span className="text-[9px] bg-red-500/15 text-red-400 px-2 py-0.5 rounded border border-red-500/30 font-bold uppercase tracking-widest animate-pulse flex items-center gap-1 leading-none font-mono">
                      <span className="h-2 w-2 bg-red-500 rounded-full inline-block animate-ping" /> Listening
                    </span>
                  )}
                </div>
                ...
                <div className="bg-black/50 border border-neutral-900 rounded-lg p-3 h-36 overflow-y-auto space-y-3 scrollbar-thin scrollbar-thumb-zinc-800">
                  ...
                  {mentalChatLoading && (
                    <div className="text-[10px] text-slate-500 italic animate-pulse">Counselor typing personalized guide insights...</div>
                  )}
                </div>
                ...
                <div className="flex gap-2">
                  <input
                    type="text"
                    placeholder="Describe stress, mood, or speak here..."
                    value={mentalInput}
              ...
              <div className="bg-[#0C0C0F] border border-purple-500/5 rounded-xl p-4 space-y-3">
                <h4 className="text-xs font-black tracking-widest text-purple-300 uppercase flex items-center gap-1.5 pb-1">
                  <Sparkles className="h-3.5 w-3.5 text-purple-400" /> Personalized Mind Care Plan
                </h4>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-[#0A0A0A]/60 border border-[#1A1A1A] rounded-2xl p-6 shadow-xl space-y-6">
            <div>
              <h3 className="text-base font-black text-white uppercase tracking-tight">{t.historicalTimeline}</h3>
              <p className="text-xs text-slate-500 mt-1">Review chronologically synced historical assessments logged to date.</p>
            </div>

            {loadingTimeline ? (
              <div className="py-12 text-center text-xs text-slate-505 font-medium flex items-center justify-center gap-2">
                <RefreshCw className="h-4.5 w-4.5 animate-spin text-emerald-400" /> Loaded clinical logs...
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-black/40 border border-[#1C1C1C] p-4 rounded-xl space-y-3">
                  <div className="flex justify-between items-center bg-zinc-950/40 p-1.5 rounded border border-[#151515]">
                    <span className="text-[9px] text-slate-550 font-mono uppercase tracking-widest font-black">Trending Progression Curve</span>
                    <span className="text-[9px] bg-emerald-500/10 text-emerald-400 px-1.5 rounded font-bold font-mono">
                      {timelineRecords.length} sessions logged
                    </span>
                  </div>
                        ...
                        >
                          ...
                          <div className={`h-2.5 w-2.5 rounded-full border-2 transition-all ${
                            active
                              ...
                              : "bg-[#0A0A0A] border-[#333] group-hover:border-emerald-500"
                          } mb-2`} />
                  ...
                  <div className="flex justify-between text-[8px] text-slate-600 font-mono px-1">
                    <span>⏮️ Chronological Start</span>
                    <span>Recent Date Log ⏭️</span>
                  </div>
                </div>
                      ...
                      >
                        <div className="space-y-1">
                          ...
                          <span className="text-[10px] text-slate-500 font-mono">
                            Age: {rec.basicInfo.age} | BMI: {rec.basicInfo.bmi} | Sleep: {rec.lifestyle.sleepDuration}h
                          </span>
                        </div>
