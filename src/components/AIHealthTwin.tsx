...
import { 
  ...
  Clock, Flame, HelpCircle, ArrowRight, Table, Info
} from "lucide-react";
import { 
  ...
  if (!selectedRecord || !evaluation) {
    return (
      <div className="bg-[#0A0A0A] border border-[#1A1A1A] rounded-2xl p-10 text-center space-y-4">
        ...
        <h3 className="text-sm font-black uppercase text-white tracking-wider">No Active Digital Replica Sync Case</h3>
        <p className="text-xs text-slate-500 max-w-sm mx-auto">
          Please complete your initial biomarkers and health metrics from the "Health Data Input" page to assemble your first Medical Twin representation.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6" id="ai-health-twin-portal-panel">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-5 relative z-10">
          <div className="space-y-1.5 text-center md:text-left">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
              ...
              <span className="text-[9px] text-emerald-400 font-extrabold font-mono tracking-wider uppercase">AI HEALTH TWIN SIMULATION</span>
            </div>
            <h1 className="text-xl sm:text-2xl font-black text-white mt-1 uppercase tracking-tight">Meet Your Digital Health Twin</h1>
            <p className="text-xs text-slate-400 font-medium max-w-xl italic">
              "See how today's choices shape tomorrow's health."
            </p>
          </div>
          ...
          <div className="lg:col-span-5 bg-[#08080B] border border-[#16161B] rounded-2xl p-5 space-y-5 shadow-xl relative overflow-hidden">
            <div className="border-b border-[#141419] pb-3">
              <h3 className="text-xs font-black uppercase text-slate-300 tracking-wider flex items-center gap-2">
                ...
                Configure Lifestyle Sliders
              </h3>
              <p className="text-[10px] text-slate-500">Diverge simulated indicators from your current baseline clinical records</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5 p-3 rounded-xl bg-neutral-900/30 border border-neutral-900/50">
                <div className="flex justify-between items-center text-xs">
                  <span className="font-bold text-slate-300">⚖️ Body Weight</span>
                  <span className="font-mono text-emerald-400 font-bold">{simulatedWeight} kg <span className="text-[10px] text-slate-500">(BMI: {getBmi(simulatedWeight).toFixed(1)})</span></span>
                </div>
                <input
                  type="range"
                  min="40"
              ...
              <div className="space-y-1.5 p-3 rounded-xl bg-neutral-900/30 border border-neutral-900/50">
                <div className="flex justify-between items-center text-xs">
                  <span className="font-bold text-slate-300">🏋️ Exercise Frequency</span>
                  <span className="font-mono text-emerald-400 font-bold">{simulatedExercise} Days/Week</span>
                </div>
                <input
                  type="range"
                  min="0"
                ...
                />
                <div className="flex justify-between text-[8px] text-slate-555 font-mono">
                  <span>Baseline: {baselineExercise} Days</span>
              ...
              <div className="space-y-1.5 p-3 rounded-xl bg-neutral-900/30 border border-neutral-900/50">
                <div className="flex justify-between items-center text-xs">
                  <span className="font-bold text-slate-300">💤 Sleep Duration</span>
                  <span className="font-mono text-emerald-400 font-bold">{simulatedSleep} Hours/Night</span>
                </div>
                <input
                  type="range"
                  min="4"
                ...
                />
                <div className="flex justify-between text-[8px] text-slate-555 font-mono">
                  <span>Baseline: {baselineSleep} hrs</span>
              ...
              <div className="space-y-1.5 p-3 rounded-xl bg-neutral-900/30 border border-neutral-900/50">
                <div className="flex justify-between items-center text-xs">
                  <span className="font-bold text-slate-300">🧘 Stress / Allostatic Load</span>
                  <span className="font-mono text-emerald-400 font-bold">{simulatedStress} / 10 Scale</span>
                </div>
                <input
                  type="range"
                  min="1"
                ...
                />
                <div className="flex justify-between text-[8px] text-slate-555 font-mono">
                  <span>Baseline: {baselineStress} stress</span>
              ...
              <div className="space-y-1.5 p-3 rounded-xl bg-neutral-900/30 border border-neutral-900/50">
                <div className="flex justify-between items-center text-xs">
                  <span className="font-bold text-slate-300">🚭 Smoking Habit</span>
                  <select
                ...
                <div className="flex justify-between text-[8px] text-slate-555 font-mono mt-1">
                  <span>Baseline: {baselineSmoking}</span>
                  {simulatedSmoking !== baselineSmoking && (
                    <span className="text-emerald-405 font-black uppercase">Modified</span>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-1.5 p-3 rounded-xl bg-neutral-900/30 border border-neutral-900/50">
                <div className="flex justify-between items-center text-xs">
                  <span className="font-bold text-slate-300">💧 Daily Hydration</span>
                  <span className="font-mono text-emerald-400 font-bold">{simulatedWater} Liters / Day</span>
                </div>
                <input
                  type="range"
                  min="1"
                ...
                />
                <div className="flex justify-between text-[8px] text-slate-555 font-mono">
                  <span>Baseline: {baselineWater} L</span>
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-[#08080B] border border-[#16161B] rounded-2xl p-5 flex flex-col justify-between items-center text-center shadow-xl">
                <span className="text-[10px] font-black uppercase text-slate-500 tracking-wider">Twin Health Score Deflection</span>
                ...
                <div className="relative flex items-center justify-center my-4 h-32 w-32">
                  ...
                  <div className="absolute h-26 w-26 rounded-full bg-[#050508] border border-[#14141B] flex flex-col items-center justify-center">
                    <span className="text-[9px] text-slate-500 uppercase tracking-widest leading-none font-mono">Simulated</span>
                    <strong className={`text-4xl font-extrabold font-mono mt-1 ${scoreCategoryColor(results.scoreAfter)}`}>
                      {results.scoreAfter}
                    </strong>
                    <span className="text-[8px] text-slate-400 leading-none mt-1 uppercase font-mono">/100 pts</span>
                  </div>
                </div>

                <div className="w-full flex items-center justify-around border-t border-[#121217] pt-3.5 text-xs">
                  <div>
                    <span className="text-[9px] text-slate-500 uppercase block font-mono">Baseline</span>
                    <strong className="text-white block font-mono">{currentScore}</strong>
                  </div>
                  <div className="h-6 w-[1px] bg-neutral-800" />
                  <div>
                    <span className="text-[9px] text-slate-500 uppercase block font-mono">Variance</span>
              ...
              <div className="bg-[#08080B] border border-[#16161B] rounded-2xl p-5 space-y-4 shadow-xl">
                <span className="text-[10px] font-black uppercase text-slate-500 tracking-wider block">Real-time Risk Deflections</span>
            ...
            <div className="bg-[#08080B] border border-[#16161B] rounded-2xl p-5 space-y-3 shadow-xl relative overflow-hidden">
              ...
              <div className="flex items-center gap-2 border-b border-[#141419] pb-2.5">
                ...
                <div>
                  <h4 className="text-xs font-black uppercase text-white tracking-widest font-mono">Dynamic AI insights</h4>
                  <p className="text-[9px] text-slate-500 leading-none">Physiological twin variance & biochemical correlations</p>
                </div>
              </div>
              ...
              {results.timelineEstimate && (
                <div className="bg-emerald-500/5 text-emerald-400 border border-emerald-500/10 p-3 rounded-xl text-[10px] font-mono leading-relaxed mt-1 block">
                  <span className="font-extrabold uppercase mr-1.5 text-emerald-400 tracking-wider">Convergence Matrix:</span>
            ...
            <div className="bg-[#08080B] border border-[#16161B] rounded-2xl p-5 space-y-4 shadow-xl">
              <span className="text-[10px] font-black uppercase text-slate-500 tracking-wider block">Meet Your Future Self</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5 pt-1">
                ...
                <div className="bg-zinc-950 border border-neutral-900 rounded-xl p-3 text-center space-y-1.5 flex flex-col justify-between">
                  <div>
                    <span className="text-[9px] bg-zinc-900 border border-neutral-800 text-slate-400 px-2.5 py-0.5 rounded-md font-mono uppercase tracking-wider inline-block">
                      Current You
                    </span>
                    <strong className="block text-2xl font-black font-mono text-white mt-1.5">{currentScore}</strong>
                    <span className="text-[9px] text-slate-500 block leading-tight font-medium mt-1">
                      Baseline high-fidelity medical digital twin synchronized from record.
                    </span>
                  </div>
                  <span className="text-[9px] font-extrabold text-[#71717A] uppercase tracking-wider block mt-2 pt-2 border-t border-neutral-950">Normal Stasis</span>
                </div>

                {/* Future Unchanged */}
                <div className="bg-red-500/5 border border-red-950/20 rounded-xl p-3 text-center space-y-1.5 flex flex-col justify-between">
                  <div>
                    <span className="text-[9px] bg-red-950/40 border border-red-900/30 text-red-300 px-2.5 py-0.5 rounded-md font-mono uppercase tracking-wider inline-block">
                      Future (Unchanged)
                    </span>
                    <strong className="block text-2xl font-black font-mono text-red-400 mt-1.5">{Math.max(30, currentScore - 9)}</strong>
                    <span className="text-[9px] text-red-200/50 block leading-tight font-medium mt-1">
                      Estimated risk progression if current lifestyle and baseline habits remain uncorrected at Year 5.
                    </span>
                  </div>
                  <span className="text-[9px] font-black text-red-400 uppercase tracking-widest block mt-2 pt-2 border-t border-neutral-950">Risk Escalation</span>
                </div>
                ...
                <div className="bg-emerald-500/5 border border-emerald-950/20 rounded-xl p-3 text-center space-y-1.5 flex flex-col justify-between">
                  <div>
                    <span className="text-[9px] bg-emerald-950/40 border border-emerald-900/30 text-emerald-300 px-2.5 py-0.5 rounded-md font-mono uppercase tracking-wider inline-block">
                      Future (Improved)
                    </span>
                    <strong className="block text-2xl font-black font-mono text-emerald-400 mt-1.5">{results.scoreAfter}</strong>
                    <span className="text-[9px] text-emerald-200/50 block leading-tight font-medium mt-1">
                      Predicted metabolic restabilization and optimal cellular aging if targets are sustained at Year 5.
                    </span>
                  </div>
                  <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest block mt-2 pt-2 border-t border-neutral-950">Optimum Health</span>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 bg-[#08080B] border border-[#16161B] rounded-2xl p-5 space-y-5 shadow-xl">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-[#141419] pb-3">
              <div>
                <h3 className="text-xs font-black uppercase text-slate-300 tracking-wider flex items-center gap-2">
                  ...
                  Visual Health Progression Charts (5-Year Forecast)
                </h3>
                <p className="text-[10px] text-slate-500 font-medium">Evaluate path deflections based on active behavioral sandboxing</p>
              </div>
            ...
            <div className="bg-neutral-900/20 border border-neutral-900 p-3.5 rounded-xl text-[10px] leading-relaxed text-slate-400">
              <div className="flex gap-1.5 items-center text-slate-300 font-bold mb-1 uppercase text-[9px] tracking-wider">
                <Info className="h-3.5 w-3.5 text-emerald-405 shrink-0" />
                Understanding your deflection curves
              </div>
              The <strong className="text-emerald-400 font-bold">Improved curve</strong> maps predictive progress assuming your configured sliders are sustained. The <strong className="text-red-400 font-bold">Current curve</strong> tracks how allostatic load, sub-optimal resting matrices and weight profiles compounding over 60 months typically accelerate vascular decay and metabolic resistance according to mathematical random forest warning models.
            </div>
          </div>
          ...
          <div className="lg:col-span-5 bg-[#08080B] border border-[#16161B] rounded-2xl p-5 space-y-4 shadow-xl flex flex-col justify-between">
            <div className="space-y-1 pb-1 border-b border-[#141419]">
              ...
              <p className="text-[10px] text-slate-500">Summary side-by-side numerical comparison metrics</p>
            </div>
            ...
            <div className="divide-y divide-[#141419] text-xs font-semibold">
              ...
              <div className="py-3 flex justify-between items-center text-slate-300">
                <span className="flex items-center gap-1.5">
                  <span className="h-1.5 w-1.5 bg-emerald-400 rounded-full" /> Overall Health Score
                </span>
                <div className="grid grid-cols-3 gap-6 font-mono text-center text-[11px]">
                  <div>
                    <span className="text-[8px] text-slate-555 block uppercase font-mono leading-none">Current</span>
                    <strong className="text-slate-400">{currentScore}</strong>
                  </div>
                  <div>
                    <span className="text-[8px] text-slate-555 block uppercase font-mono leading-none">Unchanged</span>
                    <strong className="text-red-400">{Math.max(30, currentScore - 9)}</strong>
                  </div>
                  <div>
                    <span className="text-[8px] text-slate-555 block uppercase font-mono leading-none">Improved</span>
              ...
              <div className="py-3 flex justify-between items-center text-slate-300">
                <span className="flex items-center gap-1.5">
                  <span className="h-1.5 w-1.5 bg-teal-400 rounded-full" /> Diabetes Risk
                </span>
                <div className="grid grid-cols-3 gap-6 font-mono text-center text-[11px]">
                  <div>
              ...
              <div className="py-3 flex justify-between items-center text-slate-300">
                <span className="flex items-center gap-1.5">
                  <span className="h-1.5 w-1.5 bg-purple-400 rounded-full" /> Heart Disease Risk
                </span>
                <div className="grid grid-cols-3 gap-6 font-mono text-center text-[11px]">
                  <div>
              ...
              <div className="py-3 flex justify-between items-center text-slate-300">
                <span className="flex items-center gap-1.5">
                  <span className="h-1.5 w-1.5 bg-amber-400 rounded-full" /> Hypertension Risk
                </span>
                <div className="grid grid-cols-3 gap-6 font-mono text-center text-[11px]">
                  <div>
            ...
            <div className="mt-2 bg-gradient-to-r from-red-500/5 to-transparent border-l-2 border-red-500 p-3 rounded-r-xl text-[10px] font-medium leading-relaxed text-slate-405">
              Warning: If habits persist unchanged over 36–60 months, clinical algorithms estimate your cumulative allostatic markers could shift your digital stasis to critical thresholds, decreasing your longevity score index.
            </div>
          </div>
      ...
      <div className="bg-[#08080B] border border-[#16161B] rounded-3xl p-6 space-y-4 shadow-xl">
        <div>
          <span className="text-[9px] text-emerald-400 font-bold uppercase tracking-widest block font-mono">INTEGRATED CELLULAR RECOVERY PATH</span>
          <h3 className="text-sm font-black text-white mt-1 uppercase tracking-tight flex items-center gap-1.5">
            <Award className="h-4.5 w-4.5 text-emerald-500" /> Prevention Roadmap & Risk Deflection Milestones
          </h3>
          <p className="text-xs text-slate-500 mt-0.5">A month-by-month progressive lifestyle optimization protocol with estimated score bonuses</p>
        </div>
          ...
          <div className="bg-zinc-950 border border-neutral-900 rounded-2xl p-4.5 relative overflow-hidden flex flex-col justify-between">
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-[10px] font-mono text-emerald-400 font-bold">+4 pts</span>
              </div>
              <h4 className="text-xs font-black text-white uppercase mt-1">Foundation Steps</h4>
              <p className="text-slate-400 text-[10px] leading-relaxed">
                Aim to walk at least 5k steps daily and terminate high-fructose sugary beverages.
              </p>
            </div>
            <div className="border-t border-neutral-900/60 pt-2.5 mt-3 text-[9px] font-mono font-medium text-slate-550 flex items-center justify-between">
              <span>Cardio Deflection:</span>
              <strong className="text-emerald-450">-4% Diabetes</strong>
            </div>
          </div>
          ...
          <div className="bg-zinc-950 border border-neutral-900 rounded-2xl p-4.5 relative overflow-hidden flex flex-col justify-between">
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-[10px] font-mono text-emerald-400 font-bold">+8 pts</span>
              </div>
              <h4 className="text-xs font-black text-white uppercase mt-1">Circadian Rest</h4>
              <p className="text-slate-400 text-[10px] leading-relaxed">
                Enforce circadian sleep hygiene, targeting minimum 7 consecutive baseline rest hours.
              </p>
            </div>
            <div className="border-t border-neutral-900/60 pt-2.5 mt-3 text-[9px] font-mono font-medium text-slate-550 flex items-center justify-between">
              <span>Vascular Deflection:</span>
              <strong className="text-emerald-455">-8% Hypertension</strong>
            </div>
          </div>
          ...
          <div className="bg-zinc-950 border border-neutral-900 rounded-2xl p-4.5 relative overflow-hidden flex flex-col justify-between">
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-[10px] font-mono text-emerald-400 font-bold">+11 pts</span>
              </div>
              <h4 className="text-xs font-black text-white uppercase mt-1">Weight Transition</h4>
              <p className="text-slate-400 text-[10px] leading-relaxed">
                Reach initial 2 kg sustained weight loss using light caloric stasis restriction methods.
              </p>
            </div>
            <div className="border-t border-neutral-900/60 pt-2.5 mt-3 text-[9px] font-mono font-medium text-slate-550 flex items-center justify-between">
              <span>Coronary Deflection:</span>
              <strong className="text-emerald-450">-7% Heart Disease</strong>
            </div>
          </div>
          ...
          <div className="bg-zinc-950 border border-neutral-900 rounded-2xl p-4.5 relative overflow-hidden flex flex-col justify-between">
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-[10px] font-mono text-emerald-400 font-bold">+25 pts</span>
              </div>
              <h4 className="text-xs font-black text-white uppercase mt-1">Target Stabilization</h4>
              <p className="text-slate-400 text-[10px] leading-relaxed">
                Stabilize body weight at dynamic target levels and complete full digital twin evaluation.
              </p>
            </div>
            <div className="border-t border-neutral-900/60 pt-2.5 mt-3 text-[9px] font-mono font-medium text-slate-550 flex items-center justify-between">
              <span>Overall Deflection:</span>
              <strong className="text-emerald-450">-25% Chronic Index</strong>
            </div>
          </div>
