    ...
    <div className="space-y-6">
        ...
        return (
          <div className="bg-[#1C0F0F] border-2 border-red-500/50 p-4 rounded-xl flex flex-col md:flex-row items-center justify-between gap-4 animate-bounce shadow-xl shadow-red-950/20">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[#FFEBEB] text-xs font-black uppercase tracking-widest">ALARM: MEDICATION TIME NOW</p>
                <h3 className="text-white text-base font-bold">{activeAlarm.medicineName} {activeAlarm.dosage}</h3>
                <p className="text-red-300 text-xs">{rela} • Scheduled: {formatTime12Hour(activeAlarm.scheduledTime)}</p>
              </div>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => handleUpdateLogStatus(activeAlarm.id, "Taken")}
                className="bg-emerald-500 hover:bg-emerald-600 text-black text-xs font-black px-4 py-2 rounded-lg cursor-pointer"
              >
                Mark Taken
              </button>
              <button
                ...
                className="bg-[#2C1818] hover:bg-[#3D2222] text-[#FF9E9E] border border-red-500/20 text-xs font-bold px-3 py-2 rounded-lg"
              >
                Snooze
              </button>
              <button
                onClick={() => handleUpdateLogStatus(activeAlarm.id, "Skipped")}
                className="bg-[#150F0F] hover:bg-black text-red-400 text-xs font-semibold px-3 py-2 rounded-lg"
              >
                Skip Dose
              </button>
            </div>
        ...
        <div className="bg-[#0F0F0F] border border-[#1E1E1E] rounded-2xl p-5 flex flex-col justify-between space-y-4">
          <div className="space-y-1">
            <h3 className="text-white text-sm font-bold flex items-center gap-2">
              <Pill className="h-4 w-4 text-emerald-400" /> Compliance Scoring
            </h3>
            <p className="text-[11px] text-slate-500">Adherence percentage drives overall health prediction indexes.</p>
          </div>
          ...
          <div className="flex items-center justify-around">
            ...
            <div className="space-y-1.5 text-left">
              <span className="text-[9px] text-slate-500 font-bold uppercase tracking-widest block">Status classification</span>
              <span className={`inline-flex px-2 py-0.5 rounded text-[10px] font-bold ${
                ...
                {stats.adherenceClassification} Routine
              </span>
          ...
          <div className="grid grid-cols-2 gap-2 pt-2 border-t border-[#151515] text-center">
            <div className="p-1.5 bg-[#080808] border border-[#151515] rounded-xl">
              ...
              <p className="text-[9px] text-slate-400">Marked Taken</p>
            </div>
            <div className="p-1.5 bg-[#080808] border border-[#151515] rounded-xl">
              <span className="text-white text-xs font-black">{stats.dosesMissed}</span>
              <p className="text-[9px] text-slate-400">Missed / Skipped</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#0F0F0F] border border-[#1E1E1E] rounded-2xl p-5 lg:col-span-2 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
            <div className="space-y-0.5">
              <h3 className="text-white text-sm font-bold flex items-center gap-2">
                <CalendarDays className="h-4 w-4 text-emerald-400" /> Adherence Logs checklist
              </h3>
              <p className="text-[11px] text-slate-500">Track doses and adjust daily compliance parameters.</p>
            </div>
            <button
            ...
            >
              <Plus className="h-3 w-3" /> Register Therapy
            </button>
          </div>
          ...
          <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
            {loading ? (
              <div className="text-center py-8 text-xs text-slate-500">Loading daily adherence checklists...</div>
            ) : logs.length === 0 ? (
              <div className="text-center py-8 border border-[#151515] border-dashed rounded-xl space-y-1">
                <p className="text-xs text-slate-400">No therapy tasks scheduled for this day.</p>
                <p className="text-[10px] text-slate-600">Ensure active dates match prescription timeline limits.</p>
              </div>
            ) : (
              logs.map((log) => (
                <div 
                  ...
                  <div className="min-w-0 pr-2">
                    ...
                    <div className="flex items-center gap-1.5 text-[10px] text-slate-400 mt-0.5 flex-wrap">
                      ...
                      {log.status === "Taken" && log.actionTime && (
                        <span className="text-slate-500">Took: {log.actionTime}</span>
                      )}
                      {log.status !== "Taken" && (
                        <span>Status: <span className="font-semibold text-slate-400 capitalize">{log.status}</span></span>
                      )}
                    </div>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#0F0F0F] border border-[#1E1E1E] rounded-2xl p-5 lg:col-span-2 space-y-3.5 flex flex-col justify-between">
          <div className="space-y-1">
            <h3 className="text-white text-sm font-bold flex items-center gap-2">
              <Sparkles className="h-4 w-4 text-emerald-400" /> AI Medication Advisory Guide
            </h3>
            <p className="text-[11px] text-slate-500">Deep biological advisory analyzing prescription sync and risk trends.</p>
          </div>

          <div className="bg-black/40 border border-[#141414] rounded-xl p-4 min-h-[140px]">
            {loadingInsights ? (
              <div className="text-center py-12 text-xs text-slate-500 animate-pulse">Running smart prevention calculations...</div>
            ) : insights.insights ? (
              ...
              <div className="text-center py-12 text-xs text-slate-500">Register daily therapies to generate personalized risk progression advisories.</div>
            )}
          </div>

          <p className="text-[10px] text-slate-500 text-center">Your personal AI medication assistant. Better adherence, better health.</p>
        </div>

        {/* Card 5: Registered Prescriptions registry list */}
        <div className="bg-[#0F0F0F] border border-[#1E1E1E] rounded-2xl p-5 space-y-4">
          <div className="space-y-1">
            <h3 className="text-white text-sm font-bold">Prescription directory</h3>
            <p className="text-[11px] text-slate-500">Active chronic condition therapies being simulated.</p>
          </div>

          <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
            {medications.length === 0 ? (
              <div className="text-center py-10 border border-[#151515] border-dashed rounded-xl text-xs text-slate-500">
                No active chronic therapies. Use Register Therapy to configure schedules.
              </div>
            ) : (
              medications.map((m) => (
                  ...
                  <div className="min-w-0">
                    ...
                    {m.times && m.times.length > 0 && (
                      <p className="text-[9px] text-slate-400 mt-0.5 font-medium">
                        Times: {m.times.map(formatTime12Hour).join(", ")}
                      </p>
                    )}
                    <p className="text-[9px] text-slate-500 mt-1 truncate">Purpose: {m.purpose}</p>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0D0D0D] border border-[#1E1E1E] w-full max-w-md rounded-2xl p-5 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between pb-2 border-b border-[#1A1A1A]">
              <h3 className="text-white text-sm font-bold flex items-center gap-2">
                <Plus className="h-4 w-4 text-emerald-400" /> Register Chronic Disease Therapy
              </h3>
              <button 
            ...
            <form onSubmit={handleCreateMedication} className="space-y-3.5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[10px] text-slate-500 font-bold uppercase">Purpose of therapy</label>
                  <input
                    type="text"
                    required
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      let dTimes = ["08:00"];
                      if (f === "Twice Daily") dTimes = ["08:00", "20:00"];
                      else if (f === "Thrice Daily") dTimes = ["08:00", "13:00", "20:00"];
                      setFormFrequency(f);
                      setFormTimes(dTimes);
                    }}
                    className="w-full bg-black border border-[#1F1F1F] focus:border-emerald-500/40 text-xs text-white px-3 py-2 rounded-lg outline-none"
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] text-slate-500 font-bold uppercase">Designated Reminders Times</label>
              ...
              <div className="pt-3 border-t border-[#1A1A1A] flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setShowAddModal(false)}
                  className="bg-transparent hover:bg-white/5 border border-[#1E1E1E] text-xs text-white font-semibold px-4 py-2 rounded-lg cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="bg-emerald-500 hover:bg-emerald-600 text-black text-xs font-bold px-5 py-2 rounded-lg cursor-pointer"
                >
                  Save therapy
                </button>
              </div>
      ...
      {showEditMedModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0D0D0D] border border-[#1E1E1E] w-full max-w-md rounded-2xl p-5 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between pb-2 border-b border-[#1A1A1A]">
              <h3 className="text-white text-sm font-bold flex items-center gap-2">
                <Edit className="h-4 w-4 text-emerald-400" /> Edit Chronic Disease Therapy
              </h3>
              <button 
                onClick={() => {
                  setShowEditMedModal(false);
      ...
      {showEditLogModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0D0D0D] border border-[#1E1E1E] w-full max-w-md rounded-2xl p-5 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between pb-2 border-b border-[#1A1A1A]">
              <h3 className="text-white text-sm font-bold flex items-center gap-2">
                <Edit className="h-4 w-4 text-emerald-400" /> Edit Checklist Scheduled Dose
              </h3>
              <button 
                onClick={() => {
                  setShowEditLogModal(false);
            ...
            <form onSubmit={handleEditLog} className="space-y-3.5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[10px] text-slate-500 font-bold uppercase">Dosage Specification</label>
                  <input
                    type="text"
                    required
                    value={editLogDosage}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] text-slate-500 font-bold uppercase">Scheduled Time</label>
                  <input
                    type="time"
                    required
                    value={editLogTime}
              ...
              <div className="space-y-1">
                <label className="text-[10px] text-slate-500 font-bold uppercase">Compliance Status</label>
                <select
                  value={editLogStatus}
                  onChange={(e) => setEditLogStatus(e.target.value as any)}
                  className="w-full bg-black border border-[#1F1F1F] focus:border-emerald-500/40 text-xs text-white px-3 py-2 rounded-lg outline-none"
                >
                  <option value="Pending">Pending Dose</option>
                  <option value="Taken">Taken Dose</option>
                  <option value="Skipped">Skipped Dose</option>
                  <option value="Snoozed">Snoozed Alarm</option>
                </select>
              </div>
