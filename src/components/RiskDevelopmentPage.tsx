  ...
  if (!evaluation || timelineRecords.length === 0) {
    return (
      <div className="bg-[#0A0A0A] border border-[#1A1A1A] rounded-2xl p-10 text-center space-y-4">
        ...
        <h3 className="text-sm font-black uppercase text-white tracking-wider">No Baseline Assessment Registered</h3>
        <p className="text-xs text-slate-500 max-w-sm mx-auto">
          Please complete your clinical vital entry inside the "Clinical Intake" screen to compile predictive risk structures.
        </p>
      </div>
    );
  }
      ...
      <div className="bg-gradient-to-r from-neutral-900 to-neutral-950 border border-[#1E1E1E] rounded-2xl p-5 flex items-center justify-between">
        <div className="space-y-1">
          <span className="text-[10px] text-red-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
            <Brain className="h-4 w-4 animate-pulse text-red-500" />
            Clinical Prognosis Engine (Explainable AI Version)
          </span>
          <h2 className="text-xl font-black text-white uppercase tracking-tight">Risk Development Curves</h2>
          <p className="text-xs text-slate-500">
            Plotting chronological disease risk progression and deviations beyond target safety references.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-[#0A0A0A] border border-[#1A1A1A] rounded-2xl p-6 space-y-4 shadow-xl">
          <div className="flex justify-between items-center pb-2 border-b border-[#161616]">
            <div>
              <h3 className="text-xs font-black uppercase text-white tracking-widest flex items-center gap-1.5">
                ...
                Chronological Development Curve
              </h3>
              <p className="text-[10px] text-slate-500">Historical disease probability trends mapped across sessions</p>
            </div>
          ...
          <div className="bg-[#050505] border border-[#161616] rounded-xl p-3.5 space-y-2">
            <h4 className="text-[10px] font-black uppercase tracking-widest text-[#E0E0E0] flex items-center gap-1">
              ...
              Dynamic Shift Insights (Explainable AI)
            </h4>
            <p className="text-[11px] text-slate-400 leading-relaxed font-sans">
              Comparing your chronological datasets gives a clear indication of metabolic resilience:
              {chartData.length >= 2 ? (
                <>
                  {" "}At <span className="text-white font-mono">{chartData[0].name} (Session 1)</span>, your {selectedDiseaseType} risk index was <span className="text-red-400 font-mono font-bold">{initialRiskVal}%</span>. 
                  By <span className="text-white font-mono">{chartData[chartData.length - 1].name} (Session {chartData.length})</span>, it altered to <span className="text-red-400 font-mono font-bold">{currentRiskVal}%</span>. 
        ...
        <div className="space-y-4">
          <h3 className="text-xs font-black uppercase text-slate-400 tracking-widest flex items-center gap-1.5 font-mono">
            ...
            Predicted Disease Classified Risk Index
          </h3>
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <span className="text-[9px] text-slate-550 block font-mono">Timeline Projection: {p.timeline}</span>
                    </div>
                  ...
                  <p className="text-[10px] text-slate-450 leading-relaxed bg-[#050505] p-2.5 rounded-lg border border-[#121212]">
                    <span className="font-bold text-white block uppercase text-[8px] tracking-wider mb-0.5 mb-1 text-emerald-400 flex items-center gap-1">
                      <Brain className="h-3 w-3" /> Explainable AI Diagnostic Rule:
                    </span>
                  ...
                  <div className="p-2 border border-dashed border-[#1C1C1C] rounded-lg text-[9px] text-slate-500 font-mono flex flex-wrap gap-1 items-center">
                    <span className="font-bold uppercase text-slate-400">Trigger Indices:</span>
