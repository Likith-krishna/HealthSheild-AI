      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-[#1A1A1A] pb-4 gap-4">
        <div>
          <h2 className="text-xl font-semibold text-white flex items-center gap-2">
            ...
            Predictive Health Digital Twin Simulator
          </h2>
          <p className="text-xs text-slate-500 mt-1">
            Simulate how clinical values and daily lifestyle overhauls shift your predictive lifetime disease curves.
          </p>
        </div>
        ...
        {simulatedTwin && (
          ...
          >
            <RefreshCw className="h-3 w-3" /> Reset Variables
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
            <Sliders className="h-3.5 w-3.5" /> Behavior Sandboxing
          </h3>
          ...
          >
            {loading ? (
              <>
                ...
                <span>Re-modeling vascular and metabolic trajectories...</span>
              </>
            ) : (
              <>
                <PlayCircle className="h-4 w-4 text-black" />
                <span>Synchronize Digital Twin State</span>
              </>
            )}
          </button>
        </div>
        ...
        <div className="lg:col-span-3 bg-[#0A0A0A] border border-[#1A1A1A] rounded-2xl p-5 flex flex-col justify-between">
          {!simulatedTwin ? (
            <div id="twin-empty-result" className="my-auto text-center space-y-3 p-6">
              ...
              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Awaiting Simulation</h4>
              <p className="text-[11px] text-slate-500 max-w-xs mx-auto leading-normal">
                Toggle structural behaviors on the left panel, and initialize modeling to overlay optimized future health score lines over your baseline.
              </p>
            </div>
          ) : (
            <div id="twin-active-result" className="space-y-5 animate-fade-in">
              <div className="flex border-b border-[#1A1A1A] pb-3 flex-wrap gap-4 items-center justify-between">
                <div>
                  <h4 className="text-xs font-bold text-emerald-400 uppercase tracking-widest">Digital Twin Scenario Executed</h4>
                  <p className="text-[10px] text-slate-500 font-medium">Comparison model overlay vs default</p>
                </div>
                <div className="text-[10px] text-emerald-400 bg-emerald-500/10 px-2.5 py-0.5 rounded-full font-bold border border-emerald-500/20">
                  Simulation: Active
                </div>
              </div>
              ...
              <div className="bg-[#0F0F0F] border border-[#1A1A1A] rounded-xl p-4 flex items-center justify-around">
                <div className="text-center">
                  <span className="text-[9px] text-slate-500 uppercase font-bold tracking-wider">Before Score</span>
                  <div className="text-xl font-extrabold text-slate-400 font-mono mt-1">{simulatedTwin.overallHealthScoreBefore}</div>
                  <div className="text-[9px] text-slate-500 font-medium font-mono">Standard Life</div>
                </div>
                ...
                <div className="flex flex-col items-center">
                  <div className="text-xs font-bold text-emerald-400 flex items-center gap-0.5 font-mono">
                    ...
                    <span>+{simulatedTwin.overallHealthScoreAfter - simulatedTwin.overallHealthScoreBefore} pts</span>
                  </div>
                ...
                <div className="text-center">
                  <span className="text-[9px] text-slate-500 uppercase font-bold tracking-wider">After Score</span>
                  <div className="text-2xl font-extrabold text-emerald-400 font-mono mt-0.5">{simulatedTwin.overallHealthScoreAfter}</div>
                  <div className="text-[10px] text-emerald-400 font-extrabold uppercase mt-0.5">Optimum State</div>
                </div>
              </div>

              {/* Diseases Risks Impact overlay */}
              <div className="space-y-2.5">
                <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider block">Risk Projection Deflections:</span>
                      ...
                      <div key={idx} className="bg-[#0F0F0F] border border-[#1A1A1A] rounded-xl p-3 flex items-center justify-between text-xs hover:border-[#1E1E1E] transition-colors">
                        <div className="space-y-0.5">
                          ...
                          <span className="text-[9px] text-slate-400 font-medium block">
                            Baseline: {imp.beforeProbability}% | Optimized: {imp.afterProbability}%
                          </span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          ...
                          {diff > 0 ? (
                            ...
                            <span className="text-[10px] text-slate-500 font-medium px-1.5 py-0.5">No deflection</span>
                          )}
                        </div>
                      </div>
                    );
              ...
              {simulatedTwin.timelineEstimate && (
                <div className="text-[10px] bg-emerald-500/5 text-slate-300 border border-emerald-500/10 p-3 rounded-xl leading-relaxed mt-2">
                  <span className="font-bold block uppercase tracking-wider text-[9px] mb-0.5 text-emerald-405">Estimated physical Convergence Target:</span>
                  {simulatedTwin.timelineEstimate}
                </div>
              )}
            </div>
