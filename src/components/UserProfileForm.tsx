    ...
    <div id="user-profile-form" className="bg-[#0F0F0F] border border-[#1A1A1A] rounded-2xl shadow-xl shadow-black/40 p-6 space-y-6">
      <div className="flex items-center justify-between border-b border-[#1A1A1A] pb-4">
        <div>
          <h2 className="text-xl font-semibold text-white flex items-center gap-2">
            <User className="h-5 w-5 text-emerald-400" />{t("auto.vitals_biomarkers_intake", "Vitals & Biomarkers Intake")}

          </h2>
          <p className="text-sm text-slate-500 mt-1">{t("auto.provide_baseline_lab_details_or_load_sam", "Provide baseline lab details or load sample files inside the report room.")}

          </p>
        </div>
        <div className={`text-xs px-3 py-1 rounded-full font-medium flex items-center gap-1.5 ${bmiStatus.color}`}>
          <Flame className="h-3.5 w-3.5" />
          <span>{t("auto.calculated_bmi", "Calculated BMI:")}{bmi} ({bmiStatus.text})</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
            <User className="h-3.5 w-3.5" />{t("auto.demographic_metrics", "Demographic Metrics")}
          </h3>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">{t("auto.age", "Age")}</label>
              <input
                id="profile-age"
                type="number"
                value={profile.age}
            ...
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">{t("auto.gender", "Gender")}</label>
              <select
                id="profile-gender"
                ...
                <option value="Male" className="bg-[#050505] text-white">{t("auto.male", "Male")}</option>
                <option value="Female" className="bg-[#050505] text-white">{t("auto.female", "Female")}</option>
                <option value="Other" className="bg-[#050505] text-white">{t("auto.other", "Other")}</option>
              </select>
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">{t("auto.heart_rate", "Heart Rate")}</label>
              <input
                id="profile-hr"
                type="number"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">{t("auto.height_cm", "Height (cm)")}</label>
              <input
                id="profile-height"
                type="number"
            ...
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">{t("auto.weight_kg", "Weight (kg)")}</label>
              <input
                id="profile-weight"
                type="number"
        ...
        <div className="space-y-4">
          <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
            <Heart className="h-3.5 w-3.5 text-rose-500" />{t("auto.cardiovascular_metabolic_markers", "Cardiovascular & Metabolic Markers")}
          </h3>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1 flex items-center justify-between">
                <span>{t("auto.systolic_bp", "Systolic BP")}</span>
                <span className="text-[10px] text-slate-500 font-normal">{t("auto.normal_120", "Normal < 120")}</span>
              </label>
              <div className="relative">
                ...
                <span className="absolute right-3 top-2 text-[10px] text-slate-500 uppercase font-medium">{t("auto.mmhg", "mmHg")}</span>
              </div>
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1 flex items-center justify-between">
                <span>{t("auto.diastolic_bp", "Diastolic BP")}</span>
                <span className="text-[10px] text-slate-500 font-normal">{t("auto.normal_80", "Normal < 80")}</span>
              </label>
              <div className="relative">
                <input
                  id="profile-dia-bp"
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-2">
              <label className="block text-xs font-medium text-slate-500 mb-1 flex items-center justify-between">
                <span>{t("auto.fasting_glucose", "Fasting Glucose")}</span>
                <span className="text-[10px] text-slate-500 font-normal">{t("auto.healthy_100", "Healthy < 100")}</span>
              </label>
              <div className="relative">
                <input
                  id="profile-sugar"
                ...
                <span className="absolute right-3 top-1.5 text-[10px] text-slate-500 font-medium">{t("auto.mg_dl", "mg/dL")}</span>
              </div>
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1 flex items-center justify-between">
                <span>{t("auto.hba1c", "HbA1c %")}</span>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-500 mb-1 flex items-center justify-between">
              <span>{t("auto.total_serum_cholesterol", "Total Serum Cholesterol")}</span>
              <span className="text-[10px] text-slate-500 font-normal">{t("auto.ideal_200_mg_dl", "Ideal < 200 mg/dL")}</span>
            </label>
            <div className="relative">
              <input
                id="profile-chol"
              ...
              <span className="absolute right-3 top-2 text-[10px] text-slate-500 font-medium uppercase">{t("auto.mg_dl", "mg/dL")}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest flex items-center gap-1.5 text-emerald-400">
          <Activity className="h-3.5 w-3.5 text-emerald-450" />{t("auto.core_daily_habits_behavioral_matrix", "Core Daily Habits & Behavioral Matrix")}
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <label className="block text-xs font-medium text-slate-500 mb-1">{t("auto.tobacco_exposure", "Tobacco Exposure")}</label>
            <select
              id="profile-smoking"
              ...
              <option value="Never" className="bg-[#050505] text-white">{t("auto.never_smoked", "Never Smoked")}</option>
              <option value="Former" className="bg-[#050505] text-white">{t("auto.former_smoker", "Former Smoker")}</option>
              <option value="Active" className="bg-[#050505] text-white">{t("auto.active_smoker", "Active Smoker")}</option>
            </select>
          </div>

          <div>
            <label className="block text-xs font-medium text-slate-500 mb-1">{t("auto.alcohol_consumption", "Alcohol consumption")}</label>
            <select
              id="profile-alcohol"
              ...
              <option value="Never" className="bg-[#050505] text-white">{t("auto.never", "Never")}</option>
              <option value="Social" className="bg-[#050505] text-white">{t("auto.socially_occasional", "Socially / Occasional")}</option>
              <option value="Heavy" className="bg-[#050505] text-white">{t("auto.heavy_drinker", "Heavy Drinker")}</option>
            </select>
          </div>

          <div>
            <label className="block text-xs font-medium text-slate-500 mb-1 flex justify-between">
              <span>{t("auto.sleep_hours", "Sleep Hours")}</span>
              <span className="text-[10px] text-slate-500">{t("auto.target_7_8h", "Target 7-8h")}</span>
            </label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-500 mb-1 flex justify-between">
              <span>{t("auto.exercise_days_wk", "Exercise Days / Wk")}</span>
              <span className="text-[10px] text-slate-500">{t("auto.target_3_5d", "Target 3-5d")}</span>
            </label>
            <div className="flex items-center gap-2">
              <input
                id="profile-exc"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="block text-xs font-medium text-slate-500 mb-1 flex justify-between">
              <span>{t("auto.daily_fluid_hydration", "Daily Fluid Hydration")}</span>
              <span className="text-emerald-400 text-[10px] font-medium">{profile.waterIntake}{t("auto.ml", "ml")}</span>
            </label>
            <div className="flex items-center gap-2">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-500 mb-1 flex justify-between">
              <span>{t("auto.allostatic_stress_index", "Allostatic Stress Index")}</span>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-500 mb-1">{t("auto.nutrition_style", "Nutrition Style")}</label>
            <select
              id="profile-diet"
              ...
              <option value="Standard" className="bg-[#050505] text-white">{t("auto.standard_western_diet", "Standard Western Diet")}</option>
              <option value="Mediterranean" className="bg-[#050505] text-white">{t("auto.mediterranean_cardioprotective_diet", "Mediterranean Cardioprotective Diet")}</option>
              <option value="Keto" className="bg-[#050505] text-white">{t("auto.ketogenic_high_fat_diet", "Ketogenic High-Fat Diet")}</option>
              <option value="Vegan" className="bg-[#050505] text-white">{t("auto.whole_foods_vegan_diet", "Whole Foods Vegan Diet")}</option>
              <option value="Low-Carb" className="bg-[#050505] text-white">{t("auto.low_carbohydrate_diet", "Low-Carbohydrate Diet")}</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-1">
            <Compass className="h-3.5 w-3.5" />{t("auto.first_degree_family_medical_history", "First-Degree Family Medical History")}
          </h3>
        ...
        <div>
          <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-1">
            <Activity className="h-3.5 w-3.5 text-[#E0E0E0]" />{t("auto.self_clinical_history", "Self Clinical History")}
          </h3>
          <div className="grid grid-cols-2 gap-2">
      ...
      <div className="pt-4 flex">
        <button
          ...
          <>
              ...
              <span>{t("auto.recalibrating_clinical_risk_intelligence", "Recalibrating Clinical Risk Intelligence...")}</span>
            </> :

          <>
              <Sparkles className="h-4 w-4 text-black" />
              <span>{t("auto.simulate_prognosis_prediction_index", "Simulate Prognosis Prediction Index")}</span>
            </>
          }
        </button>
      </div>
