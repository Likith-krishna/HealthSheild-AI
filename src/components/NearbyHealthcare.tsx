...
import {
  ...
  Sparkles, ShieldCheck, Heart, Stethoscope, RefreshCw, ChevronRight, Info, ExternalLink } from
"lucide-react";
...
}: {origin: {lat: number;lng: number;};destination: {lat: number;lng: number;};map: google.maps.Map | null;}) {
...
function InnerMapComponent({
...
}: {center: {lat: number;lng: number;};searchQuery: string;hospitals: Hospital[];setHospitals: React.Dispatch<React.SetStateAction<Hospital[]>>;activeHospital: Hospital | null;setActiveHospital: React.Dispatch<React.SetStateAction<Hospital | null>>;directionsDestination: {lat: number;lng: number;} | null;setDirectionsDestination: React.Dispatch<React.SetStateAction<{lat: number;lng: number;} | null>>;setSearching: React.Dispatch<React.SetStateAction<boolean>>;setError: React.Dispatch<React.SetStateAction<string>>;}) {
  const map = useMap();
  ...
  return (
    <>
      ...
      <AdvancedMarker position={center} title={t("auto.your_live_location", "Your Live Location")}>
        <Pin background="#3b82f6" glyphColor="#fff" borderColor="#1d4ed8" scale={1.2}>
          <div className="text-[9px] font-sans font-bold text-white uppercase px-1">{t("auto.me", "ME")}</div>
        </Pin>
      </AdvancedMarker>
    ...
    return (
      <div className="bg-[#0A0A0A] border border-[#1A1A1A] rounded-3xl p-8 max-w-2xl mx-auto space-y-6">
        <div className="text-center space-y-3">
          ...
          <h2 className="text-lg font-black uppercase text-white tracking-wider">{t("auto.google_maps_api_key_missing", "Google Maps API Key Missing")}</h2>
          <p className="text-xs text-slate-400 max-w-md mx-auto">{t("auto.healthsheild_ai_requires_a_registered_go", "HealthSheild AI requires a registered Google Cloud console key to activate spatial distance metrics, interactive maps, and authentic Place searches.")}
        ...
        <div className="p-5 bg-[#050505] border border-[#161616] rounded-2xl space-y-4">
          <div className="space-y-2">
            <span className="text-[10px] uppercase font-bold text-slate-500 tracking-wider">{t("auto.step_1_get_an_api_key", "Step 1: Get an API Key")}</span>
            <p className="text-xs text-slate-400 leading-relaxed">{t("auto.navigate_to_google_maps_platform_console", "Navigate to Google Maps Platform console and obtain a valid developer credential:")}
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 text-xs text-emerald-400 font-extrabold hover:underline">{t("auto.get_google_maps_api_key", "Get Google Maps API Key")}
          ...
          <div className="space-y-2.5">
            <span className="text-[10px] uppercase font-bold text-slate-500 tracking-wider">{t("auto.step_2_save_as_ai_studio_secret", "Step 2: Save as AI Studio Secret")}</span>
            <p className="text-xs text-[#999] leading-relaxed">{t("auto.input_the_credential_safely_into_your_wo", "Input the credential safely into your workspace environment parameters:")}

            </p>
            <ul className="text-[11px] text-slate-500 space-y-1.5 list-disc pl-5 font-mono">
              <li>{t("auto.open_settings_panel_gear_icon_in_top_rig", "Open settings panel (\u2699\uFE0F gear icon in top-right)")}</li>
              <li>{t("auto.seek", "Seek")}<strong>{t("auto.secrets", "Secrets")}</strong>{t("auto.menu", "menu")}</li>
              <li>{t("auto.add_key_name", "Add key name:")}<code className="bg-[#121212] px-1.5 py-0.5 rounded text-white border border-[#222]">{t("auto.google_maps_platform_key", "GOOGLE_MAPS_PLATFORM_KEY")}</code></li>
              <li>{t("auto.paste_the_api_string_token_value_apply", "Paste the API string token value & apply")}</li>
            </ul>
          </div>
        </div>

        <div className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-xl">
          <p className="text-[11px] text-emerald-400 font-mono text-center">{t("auto.the_applet_automatically_recompiles_and", "\u2022 The applet automatically recompiles and injects the telemetry upon secret assignment.")}
    ...
    <div className="space-y-6" id="nearby-healthcare-system-root">
      ...
      <div className="bg-gradient-to-r from-neutral-900 to-neutral-950 border border-[#1E1E1E] rounded-3xl p-6 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-4 shadow-xl">
        ...
        <div className="space-y-1 z-10 text-center md:text-left">
          <span className="text-[10px] text-emerald-400 font-black uppercase tracking-widest flex items-center justify-center md:justify-start gap-1.5">
            <Compass className="h-3.5 w-3.5 animate-spin-slow text-emerald-450" />{t("auto.smart_hospital_doctor_recommendation_sys", "Smart Hospital & Doctor Recommendation System")}

          </span>
          <h2 className="text-xl font-black text-white uppercase tracking-tight">{t("auto.nearby_healthcare_assistance", "Nearby Healthcare Assistance")}</h2>
          <p className="text-xs text-slate-500">{t("auto.fusing_calculated_disease_risk_projectio", "Fusing calculated disease risk projections with real-world Google Maps API and Places directory.")}

          </p>
        </div>

        <div className="shrink-0 z-10 bg-amber-500/10 border border-amber-500/25 px-4.5 py-2 rounded-xl text-center">
          <span className="text-[9px] uppercase font-bold text-slate-400 block font-mono">{t("auto.risk_profile_target", "Risk Profile Target")}</span>
      ...
      <div className="bg-gradient-to-r from-red-950/20 via-neutral-950 to-neutral-950 border border-red-500/30 rounded-2xl p-5 relative overflow-hidden flex flex-col md:flex-row justify-between items-center gap-4 shadow-xl">
          ...
          <div className="space-y-1.5 z-10 text-center md:text-left">
            <div className="text-[10px] text-red-500 font-black uppercase tracking-widest flex items-center justify-center md:justify-start gap-1 font-mono">
              <AlertTriangle className="h-4 w-4 text-red-500 animate-pulse" />{t("auto.critical_health_risk_detected", "Critical Health Risk Detected")}

          </div>
            <h3 className="text-sm font-black text-white uppercase tracking-tight">{t("auto.elevated", "Elevated")}
            {recommendedSpecialistTitle}{t("auto.consult_advised", "Consult Advised")}
          </h3>
            <p className="text-xs text-slate-400 max-w-2xl leading-relaxed">{t("auto.this_prediction_indicates_a_significantl", "This prediction indicates a significantly elevated health risk. Consult a qualified healthcare professional as soon as possible. Nearby hospitals and specialists are mapped below.")}

          </p>
          </div>
          <div className="shrink-0 z-10 bg-red-500/10 border border-red-500/25 px-4 py-2.5 rounded-xl text-center">
            <span className="text-[9px] uppercase font-bold text-slate-400 block font-mono">{t("auto.specialist_recommended", "Specialist Recommended")}</span>
      ...
      <div className="bg-[#0A0A0A]/80 border border-emerald-500/10 rounded-2xl p-4.5 flex justify-between items-center gap-4">
          <div className="space-y-0.5 flex items-center gap-3">
            ...
            <div>
              <h4 className="text-xs font-black uppercase text-white tracking-widest">{t("auto.normal_preventive_profile", "Normal Preventive Profile")}</h4>
              <p className="text-[11px] text-slate-600 font-medium">{t("auto.standard_health_scores_catalogued_displa", "Standard health scores catalogued. Displaying nearby hospitals for general convenience.")}</p>
            </div>
          </div>
          <span className="text-[10px] bg-emerald-500/5 border border-emerald-500/12 text-emerald-400 px-2.5 py-1 rounded-full uppercase font-bold font-mono">{t("auto.clinically_safe", "Clinically Safe")}</span>
        </div>
      }
      ...
      <div className="bg-[#0A0A0A] border border-[#1A1A1A] p-5 rounded-2xl space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#151515] pb-3">
          <div>
            <h3 className="text-xs font-black uppercase text-white tracking-wider flex items-center gap-1.5">
              <MapPin className="h-4 w-4 text-emerald-400" />{t("auto.user_location_configuration", "User Location Configuration")}

            </h3>
            <p className="text-[10px] text-slate-500">{t("auto.configure_geolocation_sensors_or_calibra", "Configure geolocation sensors or calibrate standard physical coordinates below.")}</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="space-y-1.5">
            <label className="text-slate-400 text-[10px] font-bold uppercase tracking-wider block">{t("auto.house_flat_no", "House/Flat No")}</label>
            <input
              type="text"
              value={manualHouse}
              onChange={(e) => setManualHouse(e.target.value)}
              placeholder={t("auto.e_g_4b", "e.g. 4B")}
          ...
          <div className="space-y-1.5">
            <label className="text-slate-400 text-[10px] font-bold uppercase tracking-wider block">{t("auto.street_name_area", "Street Name/Area")}</label>
            <input
              type="text"
              value={manualStreet}
              onChange={(e) => setManualStreet(e.target.value)}
              placeholder={t("auto.e_g_5th_main_st", "e.g. 5th Main St")}
              className="w-full bg-black border border-[#1E1E1E] focus:border-emerald-500/40 text-xs text-white px-3.5 py-2.5 rounded-xl outline-none" />
            
          </div>
          <div className="space-y-1.5">
            <label className="text-slate-400 text-[10px] font-bold uppercase tracking-wider block">{t("auto.city_region", "City/Region")}</label>
            <input
              type="text"
              value={manualArea}
              onChange={(e) => setManualArea(e.target.value)}
              placeholder={t("auto.e_g_chennai_tn", "e.g. Chennai, TN")}
              className="w-full bg-black border border-[#1E1E1E] focus:border-emerald-500/40 text-xs text-white px-3.5 py-2.5 rounded-xl outline-none" />
        ...
        <div className="p-3 bg-black/60 border border-[#141414] rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-slate-500 font-mono">{t("auto.location_status", "Location Status:")}</span>
            <span className="text-[10px] text-slate-300 font-mono font-bold">{userLocationSource}</span>
          </div>

          <div className="flex items-center gap-3 text-right">
            <div className="text-[10px] text-slate-500 font-mono">{t("auto.latitude", "Latitude:")}
              <span className="text-emerald-400 font-bold">{coordinates.lat.toFixed(4)}</span>
            </div>
            <div className="text-[10px] text-slate-500 font-mono">{t("auto.longitude", "Longitude:")}
              <span className="text-emerald-400 font-bold">{coordinates.lng.toFixed(4)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-5 bg-[#0A0A0A] border border-[#1A1A1A] p-5 rounded-3xl space-y-4 shadow-xl">
          <div className="flex justify-between items-center border-b border-[#151515] pb-3.5">
            <div>
              <h3 className="text-xs font-black uppercase text-white tracking-widest">{t("auto.recommended_real_facilities", "Recommended Real Facilities")}</h3>
              <p className="text-[10px] text-slate-500">{t("auto.verified_locations_from_real_time_google", "Verified locations from real-time Google Places queries.")}</p>
            </div>
            
            <span className="text-[10px] bg-neutral-900 border border-[#222] text-slate-400 font-mono py-1 px-2.5 rounded-lg">{t("auto.showing_1_8_center_results", "Showing 1-8 Center results")}
          ...
          <div className="bg-black/40 border border-[#181818] p-3 rounded-xl flex items-center gap-2">
            <span className="text-[10px] uppercase font-bold text-slate-500 font-mono shrink-0 pl-1">{t("auto.search_focus", "Search Focus:")}</span>
            <select
              ...
              <option value="Multi-Speciality Hospital">{t("auto.multi_speciality_hospitals_general_check", "Multi-Speciality Hospitals (General Check)")}</option>
              <option value="Emergency Care Hospital">{t("auto.emergency_care_centers", "Emergency Care Centers")}</option>
              <option value="Cardiologist Cardiology Cardiovascular Department">{t("auto.cardiology_heart_hypertension", "Cardiology (Heart/Hypertension)")}</option>
              <option value="Endocrinologist Diabetes Metabolic Specialist Hospital">{t("auto.endocrinology_diabetes_metabolic", "Endocrinology (Diabetes/Metabolic)")}</option>
              <option value="Nephrologist Kidney Care Center Dialysis">{t("auto.nephrology_kidney_conditions", "Nephrology (Kidney Conditions)")}</option>
              <option value="Psychiatrist Mental Care Psychiatric Clinic">{t("auto.psychiatry_mental_wellness_counseling", "Psychiatry (Mental Wellness/Counseling)")}</option>
              <option value="Pulmonologist Respiratory Hospital Lungs Clinic">{t("auto.pulmonology_respiratory_asthma", "Pulmonology (Respiratory/Asthma)")}</option>
              <option value="Gastroenterologist Hepatology Clinic Liver Care">{t("auto.gastroenterology_hepatology_liver", "Gastroenterology (Hepatology/Liver)")}</option>
            </select>
          </div>
          ...
          <div className="py-12 text-center space-y-3">
              ...
              <p className="text-[10px] text-slate-400 font-mono">{t("auto.polling_google_places_metadata_for_authe", "Polling Google Places metadata for authentic results...")}</p>
            </div> :
          ...
          hospitals.length === 0 ?
          <div className="py-12 text-center text-slate-500 text-xs">{t("auto.no_matching_clinical_structures_found_in", "No matching clinical structures found in local radius. Try adjusting location filters.")}
                ...
                return (
                  <div
                    ...
                    <div className="absolute top-0 right-0 h-16 w-16 bg-[#111] rounded-bl-2xl flex items-center justify-center border-l border-b border-[#1c1c1c] group-hover:bg-[#151515] transition-colors z-10">
                      <span className="text-[10px] font-black text-emerald-400 font-mono tracking-tight">{distanceVal}{t("auto.km", "km")}</span>
                    </div>
                    ...
                    <div className="space-y-2 max-w-[80%] pr-4">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {i === 0 &&
                        <span className="text-[8px] tracking-wider font-extrabold uppercase bg-emerald-500/10 text-emerald-400 border border-emerald-500/15 px-1.5 py-0.5 rounded">{t("auto.highest_rated", "Highest Rated")}</span>
                        }
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-slate-500">{h.userRatingCount || 10}{t("auto.user_reviews", "User Reviews")}</span>
                        <span className="h-1 w-1 rounded-full bg-slate-700 shrink-0" />
                        <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider font-mono">{t("auto.hospital", "HOSPITAL")}</span>
                      </div>
                    ...
                    <div className="mt-3.5 pt-3.5 border-t border-[#1F1F1F] flex items-center justify-between gap-2">
                        ...
                        className="bg-emerald-500 text-black py-1.5 px-3.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all flex items-center gap-1 hover:opacity-95 cursor-pointer">
                          ...
                          <Navigation className="h-3.5 w-3.5" />{t("auto.plot_directions", "Plot Directions")}
                        ...
                        className="bg-neutral-900 border border-[#222] text-slate-400 py-1.5 px-3 rounded-lg text-[10px] font-extrabold uppercase hover:text-white transition-colors flex items-center gap-1 cursor-pointer">
                          ...
                          <ExternalLink className="h-3.5 w-3.5" />{t("auto.open_map", "Open Map")}
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="p-1.5 bg-[#0A0A0A] border border-[#1A1A1A] rounded-3xl shadow-xl overflow-hidden relative">
            <div className="absolute top-4 left-4 z-10 bg-[#0A0A0A]/90 border border-[#1E1E1E] px-3.5 py-1.5 rounded-xl flex items-center gap-2 shadow-lg backdrop-blur-md">
              ...
              <span className="text-[10px] text-white uppercase tracking-wider font-extrabold font-mono">{t("auto.live_google_maps_tracking", "Live Google maps tracking")}</span>
            </div>
          ...
          <div className="bg-[#0A0A0A] border border-[#1A1A1A] p-4 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 text-slate-500 font-mono text-[10px]">
            <span>{t("auto.powered_by", "Powered by:")}<strong>{t("auto.places_api_new_directions_sdk", "Places API (New) & Directions SDK")}</strong></span>
            <span>{t("auto.radius_index", "Radius Index:")}<strong>{t("auto.15_000_meters_bounds", "<15,000 meters bounds")}</strong></span>
          </div>
        </div>
