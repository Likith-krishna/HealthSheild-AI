        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-mono font-extrabold tracking-widest text-emerald-400 bg-emerald-500/10 border border-emerald-505/20 rounded-full uppercase">
              <Award className="h-3.5 w-3.5 text-emerald-450 animate-pulse" />
              Patient Age-10 Simplifier Mode Live
            </div>
            <h1 className="text-2xl md:text-3.5xl font-black text-white uppercase tracking-tight leading-none">
              Explain My Report <span className="text-emerald-400">Like I'm 10</span>
            </h1>
            <p className="text-xs md:text-sm text-slate-400 max-w-2xl font-medium">
              "Simple health. No scary jargon." Turn confusing, scientific laboratory spreadsheets and blood test values into custom-styled stories, friendly home analogies, and simple quests that anyone can understand!
            </p>
          </div>
          <div className="flex items-center gap-2 self-start md:self-center">
            <div className="bg-emerald-500/10 border border-emerald-500/20 px-4 py-2.5 rounded-2xl flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] text-slate-500 block uppercase font-bold tracking-wider">Reading Level</span>
                <span className="text-xs font-extrabold text-slate-200">Friendly Pediatric</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 bg-[#0A0A0A] border border-[#171717] rounded-3xl p-5 shadow-xl space-y-6 flex flex-col justify-between">
          <div className="space-y-6">
            <div className="border-b border-[#141414] pb-4 flex items-center justify-between">
              <div>
                <h2 className="text-sm font-black text-white uppercase tracking-wide flex items-center gap-2">
                  ...
                  1. Laboratory Data Input
                </h2>
                <p className="text-[11px] text-slate-500 mt-0.5">Upload a scanned file, choose a preset, or type values manually.</p>
              </div>
              <button 
                onClick={() => setParams([
            ...
            {/* PRESETS BUTTONS */}
            <div className="space-y-2">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Choose Demo Scenario Preset:</span>
            ...
            {/* DRAG-ZONE */}
            <div className="space-y-2">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Or upload laboratory sheet:</span>
              <div
              ...
              >
                ...
                {fileLoading ? (
                  <div className="flex flex-col items-center py-2">
                    ...
                    <span className="text-[11px] font-bold text-white">Extracting raw markers via Cognitive OCR...</span>
                  </div>
                ) : (
                  <>
                    <Upload className="h-6 w-6 text-emerald-500/50 group-hover:text-emerald-400 mb-1.5" />
                    <span className="text-xs font-bold text-white block">Drop blood work report here</span>
                    <span className="text-[10px] text-slate-500 block">or browse files (PDF, JPG, PNG, TXT, CSV)</span>
                  </>
                )}
              </div>
              {uploadError && (
            ...
            <div className="space-y-3">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Active Biomarkers For Simplify Table:</span>
              
              <div className="space-y-2 max-h-[190px] overflow-y-auto pr-1">
                {params.length === 0 ? (
                  <div className="p-4 border border-[#161616] rounded-xl text-center text-[11px] text-slate-600 font-mono">
                    Empty board. Choose a preset or type below.
                  </div>
                ) : (
                  params.map((p, idx) => (
                    <div key={idx} className="flex items-center justify-between p-2.5 rounded-xl bg-black border border-[#121215] hover:border-[#1E1E25] transition-colors">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <span className="text-[9px] text-slate-500 font-mono">Ideal range: {p.normalRange} {p.unit}</span>
                        </div>
                      </div>
              ...
              <div className="bg-[#050505] border border-[#151515] p-3 rounded-2xl space-y-2">
                <span className="text-[9px] font-black text-slate-500 uppercase tracking-wider block">Add Custom / Missing Parameter Manually:</span>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <input
                    type="text"
                    placeholder="Norm Range (e.g. < 200)"
                    value={newParamRange}
          ...
          >
            {analysisLoading ? (
              <>
                ...
                <span>Simplifying Lab Variables...</span>
              </>
            ) : (
              <>
                <Sparkles className="h-4 w-4" />
                <span>Explain Like I'm 10 (Process AI)</span>
              </>
            )}
          </button>
        </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {analysisLoading && (
              ...
              >
                ...
                <div className="space-y-1 max-w-sm">
                  <h3 className="text-sm font-black text-white uppercase tracking-wider">Cognitive simplification engine engaged</h3>
                  <p className="text-[10px] uppercase tracking-widest text-[#999] font-mono font-bold">
                    Currently: {loadingPhrases[loadingMessageIdx]}
                  </p>
                </div>
                <p className="text-xs text-slate-500 max-w-md">
                  Gemini is packaging complex biology parameters into colorful metaphors, helper teams metrics, and healthy quests! This takes just a moment...
                </p>
              </motion.div>
            )}
            ...
            {!analysisLoading && !reportResult && (
              ...
              >
                ...
                <div className="space-y-1 max-w-sm">
                  <h4 className="text-sm font-black text-white uppercase tracking-wider">No simplified report loaded</h4>
                  <p className="text-xs text-slate-400">
                    Your interactive pediatric guide card is ready to be spawned! Feed the input panel on the left and click the gold <strong className="text-emerald-450">Explain Like I'm 10</strong> button to start.
                  </p>
                </div>
                <div className="p-3 bg-emerald-500/5 border border-emerald-500/10 text-emerald-400 text-[10px] font-bold rounded-xl max-w-md leading-relaxed uppercase tracking-wider font-mono">
                  💡 Tip: Choose the "Blood Sugar & Diabetes Flag" preset on the left to see the AI report simplifier build immediately!
                </div>
              </motion.div>
            )}
              ...
              >
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-6 relative z-10">
                    ...
                    <div className="relative shrink-0 flex items-center justify-center">
                      ...
                      <div className="absolute text-center">
                        ...
                        <span className="text-[9px] text-slate-500 uppercase tracking-widest font-mono font-bold block mt-0.5">Health Point</span>
                      </div>
                    </div>

                    {/* Status Text Block */}
                    <div className="text-center sm:text-left space-y-1.5">
                      <span className="text-[10px] text-slate-500 uppercase font-black tracking-widest block font-mono">Overall Diagnostic State:</span>
                      <h2 className="text-lg md:text-xl font-extrabold text-[#E5E5E5] uppercase tracking-tight">
                        "{reportResult.overallStatusText}"
                      </h2>
                      <p className="text-[11px] text-slate-400">
                        Based on {params.length} biomarkers detected inside this medical statement. All clinical terminology has been successfully pediatric-adapted.
                      </p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-[#090C09] border border-[#161F16] rounded-2xl p-4 space-y-3 shadow shadow-emerald-900/5">
                    <h3 className="text-xs font-black text-emerald-400 uppercase tracking-widest flex items-center gap-1.5 border-b border-emerald-950/45 pb-2">
                      <CheckCircle className="h-4 w-4 shrink-0" /> Good News Checklist
                    </h3>
                  ...
                  <div className="bg-[#0D0A0A] border border-[#221616] rounded-2xl p-4 space-y-3 shadow shadow-red-950/5">
                    <h3 className="text-xs font-black text-red-400 uppercase tracking-widest flex items-center gap-1.5 border-b border-red-950/45 pb-2">
                      <AlertTriangle className="h-4 w-4 shrink-0 animate-pulse" /> Needs Caution Flags
                    </h3>
                    <ul className="space-y-2">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black text-slate-450 uppercase tracking-widest">Active Focus Quests & Recommendations:</span>
                  </div>
                        ...
                        <div key={idx} className="bg-[#0D0D11] border border-[#191924] rounded-2xl p-4 flex flex-col justify-between hover:border-emerald-500/10 transition-colors">
                          <div className="space-y-1">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-[10px] font-bold text-slate-500">Difficulty: Medium</span>
                            </div>
                ...
                <div className="bg-gradient-to-r from-red-950/15 to-neutral-900 border border-red-900/15 rounded-2xl p-4.5 space-y-2 relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-black text-red-400 uppercase tracking-widest font-mono">Kid Disease Risk Correlations Detected:</span>
                  </div>
                  <div className="space-y-2">
                    {reportResult.diseaseRisks.map((d, idx) => (
                      <div key={idx} className="bg-black/40 border border-red-500/5 p-3 rounded-xl flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                        ...
                        <div className="text-[9px] bg-red-500/10 text-red-400 border border-red-500/20 px-2 py-1 rounded-lg shrink-0 font-bold max-w-xs font-mono select-none">
                          🚨 Linked to: {d.linkage}
                        </div>
                      </div>
                    ))}
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-black text-slate-450 uppercase tracking-widest block">Detailed Biomarker Breakdown:</span>
                    <span className="text-[9px] text-[#A0A0A0] font-mono">Parameters Mapped: {reportResult.explanations.length}</span>
                  </div>
                        ...
                        >
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-3 border-b border-[#14141A]">
                            <div>
                              ...
                              <span className="text-[10px] text-slate-500 block leading-none font-mono mt-0.5">Healthy Standard: {exp.normalRange}</span>
                            </div>
                            <div className="flex items-center gap-3">
                              <div className="text-right">
                          ...
                          <div className="my-3 flex items-center gap-2 text-[10px] text-slate-500 font-mono">
                            <span className="shrink-0">Safe Limits</span>
                            ...
                            <span className="shrink-0">Over Targets</span>
                          </div>

                          {/* Body Text */}
                          <div className="space-y-3 mt-3">
                            <div className="rounded-xl border border-emerald-500/5 bg-[#0F140F]/10 p-3 leading-relaxed">
                              <span className="text-[9px] uppercase font-black text-emerald-400 block tracking-widest font-mono mb-1">Friendly Story Mapping:</span>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-medium pt-2">
                              <div>
                                <span className="text-[9px] uppercase font-black text-slate-450 block tracking-widest font-mono mb-1">Why This Matters:</span>
                                <p className="text-[11px] text-slate-400 leading-normal">{exp.impact}</p>
                              </div>
                              <div className="space-y-1">
                                <span className="text-[9px] uppercase font-black text-slate-450 block tracking-widest font-mono mb-1">Kid-Friendly Actions:</span>
                ...
                <div className="border border-[#1E1E28] bg-gradient-to-br from-[#0B0B0D] via-[#090D09] to-[#0D120D] rounded-3xl p-6 shadow-2xl relative overflow-hidden space-y-4">
                  ...
                  <div className="flex items-center justify-between border-b border-[#181822] pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h3 className="text-xs font-black text-white uppercase tracking-widest">Ask Your Friendly Doctor (AI Coach)</h3>
                        <p className="text-[10px] text-slate-500">Curious about any metric? Ask an interactive pediatric question.</p>
                      </div>
                    </div>
                    <div className="px-2.5 py-1 bg-emerald-500/5 border border-emerald-500/20 rounded-xl text-[9px] uppercase font-extrabold text-emerald-400 font-mono tracking-wider">
                      Pediatric Coach &bull; Live
                    </div>
                  </div>
                  ...
                  <div className="h-[220px] overflow-y-auto pr-1 space-y-3.5 flex flex-col">
                    ...
                    {chatLoading && (
                      <div className="self-start flex gap-3 max-w-[80%]">
                        ...
                        <div className="bg-[#111116] border border-[#14141E] rounded-2xl p-3.5 text-xs text-slate-500 shrink-0 select-none animate-pulse">
                          Friendly doctor is typing simplified answer...
                        </div>
                      </div>
                    )}
                  ...
                  <div className="space-y-1.5">
                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest block">Quick Questions To Click:</span>
                  ...
                  <form onSubmit={handleChatSubmit} className="flex gap-2 bg-[#060608] border border-[#1A1A24] rounded-2xl p-1.5">
                    <input
                      ...
                      placeholder="Ask the doctor: 'Why is bad cholesterol called sticky fat?'..."
                ...
                <div className="p-4 border border-[#1C201C]/40 bg-[#0E150E]/20 rounded-2xl flex items-start gap-3 text-[10px] text-slate-500 leading-relaxed font-sans mt-8 shadow-inner">
                  ...
                  <div>
                    <strong className="text-slate-400 font-extrabold uppercase tracking-wide block">Friendly Medical Guardrail Advice First:</strong>
                    This "Explain My Report Like I'm 10" interpreter tool prepares age-appropriate storytelling summaries purely to help understand general laboratory metrics. It should never be substituted as clinical doctor diagnostic prescriptions, treatment orders, or medical disease decisions. Always gather with your parent or dynamic pediatrician to chart your medical diagnostics maps together.
                  </div>
                </div>
