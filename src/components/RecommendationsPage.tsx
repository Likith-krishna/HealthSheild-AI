  ...
  if (!roadmap || !selectedRecord) {
    return (
      <div className="bg-[#0A0A0A] border border-[#1A1A1A] rounded-2xl p-10 text-center space-y-4">
        ...
        <h3 className="text-sm font-black uppercase text-white tracking-wider">No Active Recommendations</h3>
        <p className="text-xs text-slate-500 max-w-sm mx-auto">
          Please input baseline data in the "Clinical Intake" page to configure your preventive workout & lifestyle recommendations.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6" id="recommendations-tab-panel">
      ...
      <div className="bg-gradient-to-r from-neutral-900 to-neutral-950 border border-[#1E1E1E] rounded-2xl p-5 flex items-center justify-between">
        <div className="space-y-1">
          <span className="text-[10px] text-indigo-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
            <Dumbbell className="h-4.5 w-4.5 animate-pulse text-indigo-400" />
            Empathetic Preventive Life-Planners
          </span>
          <h2 className="text-xl font-black text-white uppercase tracking-tight">Active Recommendations</h2>
          <p className="text-xs text-slate-500 font-sans">
            Personalized lifestyle tasks, cardiorespiratory objectives, and vital target completions.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-[#0A0A0A] border border-[#1A1A1A] rounded-2xl p-6 space-y-4">
          <div>
            <h3 className="text-sm font-black uppercase text-white tracking-wider flex items-center gap-1">
              Clinical Prevention Milestones
            </h3>
            <p className="text-[11px] text-slate-500 mt-0.5">
              Check off daily target actions outlined by your biological profiles.
            </p>
          </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-[#0A0A0A] border border-[#1A1A1A] rounded-2xl p-5 space-y-4">
            <h3 className="text-xs font-black uppercase text-slate-400 tracking-widest font-mono">Dynamic Targets Tracker</h3>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="p-3 bg-rose-500/5 border border-rose-500/10 rounded-xl relative overflow-hidden">
                ...
                <span className="text-[9px] text-rose-400 font-bold uppercase">Calorie Target</span>
                <div className="text-lg font-extrabold text-white font-mono mt-0.5">{roadmap.dailyCalorieTarget}</div>
                <span className="text-[8px] text-slate-550 block mt-0.5 font-mono">kcal / daily</span>
              </div>
              ...
              <div className="p-3 bg-sky-500/5 border border-sky-500/10 rounded-xl relative overflow-hidden">
                ...
                <span className="text-[9px] text-sky-400 font-bold uppercase">Water Target</span>
                <div className="text-lg font-extrabold text-white font-mono mt-0.5">{roadmap.dailyHydrationTarget}</div>
                <span className="text-[8px] text-slate-550 block mt-0.5 font-mono">ml / daily</span>
              </div>
            </div>
            ...
            <div className="bg-[#050505] border border-[#1A1A1A] rounded-xl p-3.5 space-y-3">
              <div className="flex justify-between items-center text-[10px]">
                <span className="text-slate-400 font-bold uppercase font-sans">Fluid Compliance Tracker</span>
                <span className="text-emerald-400 font-bold font-mono">{waterProgressPct}% ({currentWaterLog}ml)</span>
              </div>
              ...
              <div className="flex gap-2.5">
                <button
                  ...
                  className="flex-1 bg-[#101010] hover:bg-[#181818] border border-[#222] text-[10px] py-2 rounded-lg font-bold text-slate-300 hover:text-white cursor-pointer transition-all uppercase"
                >
                  +250ml Glass
                </button>
                <button
          ...
          <div className="bg-[#0A0A0A] border border-[#1A1A1A] rounded-2xl p-5 space-y-3.5">
            <span className="text-xs font-black uppercase text-slate-400 tracking-wider flex items-center gap-1.5 font-mono">
              <Sparkles className="h-4.5 w-4.5 text-amber-500 animate-pulse" /> Preventative Reminders
            </span>
            <p className="text-[11px] text-slate-400 leading-normal">
              Active resistance exercises (such as lightweight sets or walking steps) recruits GLUT-4 glucose cellular receptors. This directly decreases systemic pre-diabetic risk markers. Keep hydration metrics high to protect filtering systems.
            </p>
          </div>
